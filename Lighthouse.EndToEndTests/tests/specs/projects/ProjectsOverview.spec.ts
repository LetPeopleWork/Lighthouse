...
testWithData('should show all projects on Projects Overview', async ({ testData, overviewPage }) => {
...
testWithData('should filter projects on Project Overview', async ({ testData, overviewPage }) => {
...
testWithData('should open project Edit Page when clicking on Edit Icon', async ({ testData, overviewPage }) => {
    const [project1] = testData.projects;

    const projectsPage = await overviewPage.lightHousePage.goToProjects();

    const projectDetailPage = await projectsPage.editProject(project1);
    expect(projectDetailPage.page.url()).toContain(`/projects/edit/${project1.id}`);
});

testWithData('should delete project when clicking on Delete Icon and confirming', async ({ testData, overviewPage }) => {
    const [project1] = testData.projects;
...
testWithData('should not delete project when clicking on Delete Icon and cancelling', async ({ testData, overviewPage }) => {
    const [project1] = testData.projects;
