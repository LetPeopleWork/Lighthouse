...
[
    ...
    .forEach(({ index, name }) => {
        testWithUpdatedTeams(`should allow save after validate when editing existing ${name} project`, async ({ testData, overviewPage }) => {
...
testWithData("should disable validate button if not all mandatory fields are set", async ({ testData, overviewPage }) => {
    ...
    await test.step("Work Item Query should be mandatory", async () => {
        ...
        await projectEditPage.setWorkItemQuery('[System.TeamProject] = "Lighthouse Demo" AND [System.Tags] CONTAINS "Release 1.33.7"');
        await expect(projectEditPage.validateButton).toBeEnabled();
    });
    ...
    await test.step("Unparented Work Item Query should not be mandatory", async () => {
        ...
        await projectEditPage.setUnparentedWorkItemQuery('[System.TeamProject] = "Lighthouse Demo" AND [System.Tags] CONTAINS "Release 1.33.7"');
        await expect(projectEditPage.validateButton).toBeEnabled();
...
testWithData("should handle Owning Team correctly", async ({ testData, overviewPage }) => {
    const project = testData.projects[0];
...
const newTeamConfigurations = [
    ...
    },
    {
        ...
        projectConfiguration: {
            ...
            validQuery: '[System.TeamProject] = "Lighthouse Demo" AND [System.Tags] CONTAINS "Release 1.33.7"',
            invalidQuery: '[System.TeamProject] = "Lighthouse Demo" AND [System.AreaPath] = "Lighthouse Demo\\Binary Blazers"',
            involvedTeams: [0]
        },
        workTrackingSystemOptions: [
...
newTeamConfigurations.forEach(({ name, workTrackingSystemIndex, projectConfiguration }) => {
    testWithUpdatedTeams(`should allow to create a project team for ${name}`, async ({ testData, overviewPage, request }) => {
        let projectPage = await overviewPage.lightHousePage.goToProjects();
        const newProjectPage = await projectPage.addNewProject();

        const newProject = { id: 0, name: `My New ${name} Project` };

        await test.step("Add general configuration", async () => {
            await newProjectPage.setName(newProject.name);
            await newProjectPage.setWorkItemQuery(projectConfiguration.validQuery);
        ...
        await test.step("Add Work Item Type Configuration", async () => {
            await newProjectPage.resetWorkItemTypes(['Epic'], projectConfiguration.validWorkItemTypes);
        ...
        await test.step("Add Involved Teams Configuration", async () => {
            for (const teamIndex of projectConfiguration.involvedTeams) {
        ...
        await test.step("Add State Configuration", async () => {
            await newProjectPage.resetStates(
                ...
                projectConfiguration.validStates
            );
        ...
        await test.step("Invalidate Work Item Query", async () => {
            await newProjectPage.setWorkItemQuery(projectConfiguration.invalidQuery);
            await newProjectPage.validate();
            await expect(newProjectPage.validateButton).toBeEnabled();
        ...
        await test.step("Invalidate Work Item Types", async () => {
            await newProjectPage.resetWorkItemTypes(projectConfiguration.validWorkItemTypes, projectConfiguration.invalidWorkItemTypes);
            await newProjectPage.validate();
            await expect(newProjectPage.validateButton).toBeEnabled();
            await expect(newProjectPage.saveButton).toBeDisabled();

            await newProjectPage.resetWorkItemTypes(projectConfiguration.invalidWorkItemTypes, projectConfiguration.validWorkItemTypes);
        });
        ...
        await test.step("Invalidate States", async () => {
            await newProjectPage.resetStates(projectConfiguration.validStates, projectConfiguration.invalidStates);
            await newProjectPage.validate();
            await expect(newProjectPage.validateButton).toBeEnabled();
            await expect(newProjectPage.saveButton).toBeDisabled();

            await newProjectPage.resetStates(projectConfiguration.invalidStates, projectConfiguration.validStates);
        });
...
newTeamConfigurations.forEach(({ name: workTrackingSystemName, projectConfiguration, workTrackingSystemOptions }) => {
    testWithUpdatedTeams(`should allow to create a new project with a new Work Tracking System ${workTrackingSystemName}`, async ({ testData, overviewPage, request }) => {
        test.fail(testData.projects.length < 1, "Expected to have projects initiatilized to prevent tutorial page from being displayed");

        const projectsPage = await overviewPage.lightHousePage.goToProjects();
        let newProjectPage = await projectsPage.addNewProject();

        await test.step("Add Valid Configuration for new project", async () => {
            await newProjectPage.setName(`My New ${workTrackingSystemName} Project`);
            await newProjectPage.setWorkItemQuery(projectConfiguration.validQuery);
        ...
        await test.step("Add new Work Tracking System", async () => {
            ...
            newWorkTrackingSystemDialog = await newProjectPage.addNewWorkTrackingSystem();
