...
[
    ...
    },
    {
        ...
        index: 2,
        expectedFeatures: [
            { name: 'Majestic Moments', inProgress: false, defaultSize: true, involvedTeams: [2], },
            { name: 'Astral Affinitiy', inProgress: true, defaultSize: false, involvedTeams: [2], }
        ]
    }
]
    .forEach(({ index, name, expectedFeatures }) => {
        testWithUpdatedTeams(`should show correct Features for ${name} project on refresh`, async ({ testData, overviewPage }) => {
            ...
            await test.step("Expected Features were loaded", async () => {
                for (const feature of expectedFeatures) {
                    ...
                    if (feature.inProgress) {
                        const inProgressIcon = projectDetailPage.getFeatureInProgressIcon(feature.name);
                    ...
                    for (const involvedTeamIndex of feature.involvedTeams) {
                        ...
                        const teamLink = projectDetailPage.getTeamLinkForFeature(team.name, involvedTeams[team.name].length - 1);
            ...
            await test.step("Expect Team Detail to List Features", async () => {
                for (const [team, features] of Object.entries(involvedTeams)) {
                    const teamsOverviewPage = await overviewPage.lightHousePage.goToTeams();
...
testWithData('should open Project Edit Page when clicking on Edit Button', async ({ testData, overviewPage }) => {
    ...
    expect(projectEditPage.page.url()).toContain(`/projects/edit/${project.id}`);
});

testWithUpdatedTeams(`should include milestones and WIP in feature calculation`, async ({ testData, overviewPage }) => {
    ...
    await test.step('Add Milestone', async () => {
        ...
        const milestoneColumn = projectDetailPage.getMilestoneColumn(milestoneName, milestoneDate);
        await expect(milestoneColumn).toBeVisible();

        const lastUpdatedTimeForFeature = await projectDetailPage.getLastUpdatedDateForFeature('Speedy note-taking');
    ...
    await test.step('Milestones in the past are hidden', async () => {
        ...
        const milestoneColumn = projectDetailPage.getMilestoneColumn(milestoneName, pastDate);
        await expect(milestoneColumn).not.toBeVisible();
