...
test(
	...
	async ({ page, request, overviewPage }) => {
		test.fixme(
			true,
			"BLOCKED on demo-data: demo scenario 0 seeds items with a StartedDate (Work Item Age renders) but NO state-transition history, so currentStateEnteredAt is null and every Time-in-State cell renders '—'. Un-fixme once the demo seeder populates currentStateEnteredAt on seeded in-progress items.",
		);
...
test(
	"team admin opts staleness in from the Flow Metrics Configuration group; the old Flow Signals group is gone",
	async ({ page, request, overviewPage }) => {
		await loadDemoScenario(request, DEMO_SCENARIO_ID);
		await waitForBackgroundUpdates(request);
		await page.goto("/");
...
test(
	"portfolio admin opts staleness in from the Flow Metrics Configuration group; the field is seeded with the portfolio default",
	async ({ page, request, overviewPage }) => {
		await loadDemoScenario(request, DEMO_SCENARIO_ID);
		await waitForBackgroundUpdates(request);
		await page.goto("/");
...
test(
	"flow coach sees stale items turn red in the work-item dialog after enabling staleness with a low threshold",
	async ({ page, request, overviewPage }) => {
		test.fixme(
			true,
			"BLOCKED on demo-data: same gap as the time-in-state column — demo items have no state-transition history, so currentStateEnteredAt is null, nothing is ever over-threshold, and no badge reddens. Un-fixme once the demo seeder populates currentStateEnteredAt.",
		);
		...
		await expect(workItemsDialog.timeInStateColumnHeader).toBeVisible();
		expect(
			await workItemsDialog.countStaleTimeInStateBadges(),
		).toBeGreaterThan(0);
	},
);

test(
	"flow coach sees the Time in State column on the portfolio work-item view",
	async ({ page, request, overviewPage }) => {
		test.fixme(
			true,
...
test(
	"flow coach sees the Stale Items widget count a stale item, while a blocked-and-over-threshold item is counted by Blocked not Stale",
	async ({ page, request, overviewPage }) => {
		test.fixme(
			true,
...
test(
	"flow coach sees a stale item red in the Work Item Aging Chart and its red Time in State on bubble click; a blocked-over-threshold item is never red-as-stale",
	async ({ page, request, overviewPage }) => {
		test.fixme(
			true,
