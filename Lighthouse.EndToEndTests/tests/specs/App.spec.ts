...
test('clicking contributors icon should open the contributors page', async ({ overviewPage }) => {
...
test('clicking defects icon should open the issues page', async ({ overviewPage }) => {
...
test('clicking youtube icon should open the youtube page', async ({ overviewPage }) => {
...
test('clicking rss icon should open the blog posts page', async ({ overviewPage }) => {
...
test('clicking github icon should open the github page', async ({ overviewPage }) => {
...
test('clicking email icon should open the email client', async ({ overviewPage }) => {
...
test('clicking the call button should open calendly', async ({ overviewPage }) => {
...
test('clicking the LinkedIn button should open the LPW LinkedIn Page', async ({ overviewPage }) => {
    const linkedInPage = await overviewPage.lightHousePage.contactViaLinkedIn();

    const pageTitle = await linkedInPage.title();
    expect(pageTitle).toContain('Let People Work');
    expect(linkedInPage.url()).toContain('https://www.linkedin.com/company/let-people-work');
});

test('clicking the GitHub Issue button should open the GitHub Lighthouse Issue Page', async ({ overviewPage }) => {
    const gitHubIssuePage = await overviewPage.lightHousePage.contactViaGitHubIssue();

    const pageTitle = await gitHubIssuePage.title();
    expect(pageTitle).toContain('Issues');
    expect(pageTitle).toContain('LetPeopleWork/Lighthouse');
    expect(gitHubIssuePage.url()).toBe('https://github.com/LetPeopleWork/Lighthouse/issues');
});