...
test(`Should persist Data Retention Settings Settings on Save`, async ({ overviewPage }) => {
    const settingsPage = await overviewPage.lighthousePage.goToSettings();
    let dataRetentionSettingsPage = await settingsPage.goToDataRetentionSettings();
    ...
    await test.step("Verify Changes were saved", async () => {
        ...
        const maxDays = await dataRetentionSettingsPage.getMaximumDataRetentionTime();
