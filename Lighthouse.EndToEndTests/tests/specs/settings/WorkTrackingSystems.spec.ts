...
workTrackingSystemConfiguration.forEach(({ workTrackingSystemName, workTrackingSystemOptions }) => {
    testWithData(`Should add new ${workTrackingSystemName} Work Tracking System and make it available in Team and Project creation`, async ({ testData, overviewPage }) => {
        ...
        await test.step('Create makes Work Tracking System available for teams and projects', async () => {
            ...
            const savedWorkTrackingSystem = workTrackingSystemsPage.getWorkTrackingSystem(wtsName);
            await expect(savedWorkTrackingSystem).toBeVisible();

            const newTeamPage = await (await overviewPage.lightHousePage.goToTeams()).addNewTeam();
            await newTeamPage.selectWorkTrackingSystem(wtsName);

            const newProjectPage = await (await overviewPage.lightHousePage.goToProjects()).addNewProject();
        ...
        await test.step('Delete Removes Work Tracking System', async () => {
            ...
            const removedWorkTrackingSystem = workTrackingSystemsPage.getWorkTrackingSystem(wtsName);
            await expect(removedWorkTrackingSystem).not.toBeVisible();
        });
    });
});

testWithData("Modification of Existing Work Tracking System Works as Expected", async ({ testData, overviewPage }) => {
    ...
    await test.step("Lists existing work tracking systems", async () => {
        for (const system of testData.connections) {
            const existingSystem = workTrackingSystemsPage.getWorkTrackingSystem(system.name);
    ...
    await test.step("Can't modify without providing token and Re-Validation", async () => {
        const modifyDialog = await workTrackingSystemsPage.modifyWorkTryckingSystem(connectionToModify.name);
        await expect(modifyDialog.validateButton).not.toBeEnabled();

        await modifyDialog.setWorkTrackingSystemOption('Personal Access Token', 'Bamboleo');
        ...
        await expect(workTrackingSystemsPage.getWorkTrackingSystem(oldName)).toBeVisible();
        await expect(workTrackingSystemsPage.getWorkTrackingSystem(newName)).not.toBeVisible();
    });
    ...
    await test.step("Modify name will adjust name in projects and teams", async () => {
        ...
        await modifyDialog.setWorkTrackingSystemOption('Personal Access Token', TestConfig.AzureDevOpsToken);
        await modifyDialog.setConnectionName(newName)
        await expect(modifyDialog.validateButton).toBeEnabled();
        ...
        await expect(workTrackingSystemsPage.getWorkTrackingSystem(oldName)).not.toBeVisible();
        await expect(workTrackingSystemsPage.getWorkTrackingSystem(newName)).toBeVisible();
