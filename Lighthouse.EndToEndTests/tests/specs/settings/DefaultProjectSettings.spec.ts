...
import { ProjectEditPage } from '../../models/projects/ProjectEditPage';
...
testWithRestoredDefaultSettings("Change default team settings should affect new team creation", async ({ overviewPage, defaultSettings }) => {
    ...
    const newHistoricalFeatureWorkItemQuery = "What would the elder features think about this?"
    ...
    await test.step('Adjust Settings and Save', async () => {
        ...
        const existingWorkItemTypes = ["Epic"];
        await projectSettingsPage.resetWorkItemTypes(existingWorkItemTypes, [newWorkItemType]);

        const existingToDoStates = ["New", "Proposed", "To Do"];
        const existingDoingStates = ["Active", "Resolved", "In Progress", "Committed"];
        const existingDoneStates = ["Done", "Closed"];
        await projectSettingsPage.resetStates({ toDo: existingToDoStates, doing: existingDoingStates, done: existingDoneStates }, { toDo: [newtoDoState], doing: [newDoingState], done: [newDoneState] });

        await projectSettingsPage.toggleUnparentedWorkItemConfiguration();
        await projectSettingsPage.setUnparentedWorkItemQuery(newUnparentedWorkItemQuery);

        await projectSettingsPage.toggleDefaultFeatureSizeConfiguration();
        await projectSettingsPage.useHistoricalFeatureSize();
        await projectSettingsPage.setHistoricalFeatureSizePercentile(newFeatureSizePercentile);
        await projectSettingsPage.setHistoricalFeatureSizeQuery(newHistoricalFeatureWorkItemQuery);
    ...
    async function verifyTeamSettings(projectSettings: ProjectEditPage) {
        ...
        const unparentedWorkItemQuery = await projectSettings.getUnparentedWorkItemQuery();
        ...
        await expect(projectSettings.useHistoricalFeatureSizeToggle).toBeChecked();
        
        const percentile = await projectSettings.getHistoricalFeatureSizePercentile();
        expect(percentile).toBe(newFeatureSizePercentile);

        const historicalFeatureSizeQuery = await projectSettings.getHistoricalFeatureSizeQuery();
        expect(historicalFeatureSizeQuery).toBe(newHistoricalFeatureWorkItemQuery);
    ...
    await test.step("Make sure new Projects have the default settings", async () => {
        projectSettingsPage = await overviewPage.lightHousePage.createNewProject();
