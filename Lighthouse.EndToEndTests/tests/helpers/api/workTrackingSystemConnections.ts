...
export async function createJiraConnection(api: APIRequestContext, connectionName: string): Promise<{ id: number, name: string }> {
    const options: WorkTrackingSystemOption[] = [];
    options.push({ key: 'Jira Url', value: 'https://letpeoplework.atlassian.net', isSecret: false });
    options.push({ key: 'Username', value: 'benjhuser@gmail.com', isSecret: false });
    options.push({ key: 'Api Token', value: TestConfig.JiraToken, isSecret: true });

    return createWorkTrackingSystemConnection(api, { name: connectionName, workTrackingSystem: 'Jira', options: options });
}

async function createWorkTrackingSystemConnection(api: APIRequestContext, workTrackingSystemConnectionData: {
    name: string;
