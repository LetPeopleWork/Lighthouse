...
export class WorkItemAgingChart {
	 ...
	 * than a switch per overlay. Only the modes the chart can actually paint are rendered, and
	 * pace percentiles need per-state history — so on a workflow with none, there is no control
	 * here at all rather than a control with one option.
	 */
