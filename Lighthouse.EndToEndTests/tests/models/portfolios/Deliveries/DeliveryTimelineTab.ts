...
const LEGEND_TEST_ID = "timeline-legend";
 ...
 * the parts Lighthouse owns: the tab, the chart's own wrapper, the legend, and the bar contents,
 * which are our components rendered inside the library's bars. Nothing here selects on the
 * library's own markup, which would go red on their release rather than on a defect of ours.
 */
export class DeliveryTimelineTab {
	...
	get legend(): Locator {
		return this.container.page().getByTestId(LEGEND_TEST_ID);
	}
