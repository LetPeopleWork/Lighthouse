...
- **Website freshness** — decide whether this feature changed a UI surface the public marketing website (`LetPeopleWork/website`) markets. The website hotlinks the canonical `docs/assets/**` PNGs over the jsDelivr `@main` CDN (ADR-073), so regenerating the `@screenshot` asset above auto-refreshes the site for already-covered surfaces — confirm the website still references the correct `docs/assets` path (via its `lighthouseAsset(...)` helper), and add the shot to the marketing set if the surface is newly marketed. **Record an explicit answer — `N/A, because …` when no marketed surface changed; this gate is not silently skippable.**
