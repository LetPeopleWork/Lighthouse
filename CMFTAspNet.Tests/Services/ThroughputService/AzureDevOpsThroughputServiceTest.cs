...
namespace CMFTAspNet.Tests.Services.ThroughputService
{
    ...
    {
        [Test]
        public void GetThroughput_GetsClosedItemsFromAzureDevops()
        {
            ...
            azureDevOpsWorkItemServiceMock.Setup(x => x.GetClosedWorkItemsForTeam(teamConfiguration, 7)).Returns(closedItemsPerDay);

            var subject = new AzureDevOpsThroughputService(team, azureDevOpsWorkItemServiceMock.Object);

            subject.UpdateThroughput(7);
