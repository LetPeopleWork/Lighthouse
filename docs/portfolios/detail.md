...
The burnup tells you the delivery grew. This chart tells you *which* feature grew, and *when*.

![Features over Time](../assets/features/deliveryEpicSize.png)

- Each day is one stacked bar, with a band per feature sized by the number of items it holds. A band that suddenly gets taller is scope discovered inside that feature; a band that ends is a feature that left the delivery.
- The orange **Count** line, on its own right-hand scale, is how many features were in the delivery that day. It answers a different question from the bars: three features of ten items and ten features of three items both total thirty, but they are not the same delivery to run.
- A **hatched** band means the feature's size is still the portfolio's default estimate rather than counted items. Watching a band switch from hatched to solid is watching a feature actually get broken down.

Features keep the same colour here as on the fever chart above, and hold their position in the stack from day to day, so following one feature across the window is a matter of following one colour.

Click **Legend** to open it, then click features to show only those bars — useful once a delivery carries more than a handful. The Count line is a delivery-level fact and is never filtered.

{: .note}
> This chart uses whatever you call features in **Settings → Terminology**, so it may be titled *Epics over Time*, *Initiatives over Time*, or something else on your instance.

{: .note}
> The feature **count** is drawn from the same daily snapshots the other charts use, so it goes back as far as this delivery's history. Feature **sizes** and the estimated/counted distinction were added later and are recorded from that point forward, so bars begin on the first day Lighthouse recorded them — earlier days show the count line alone. Features with no items at all are not counted.
