...
| 1.7 | Products shall process only data necessary for intended purpose | ✅ Implemented | Work tracking data is fetched only for the Teams and Portfolios an operator configured. Beyond that, an instance can report which of ten Team and Portfolio detail tabs were opened, along with its version, deployment mode, licence tier and whether authentication is switched on. That reporting is **opt-in per browser**: off until somebody agrees on that instance, revocable from the footer at any time, and an instance nobody has answered on sends nothing at all. Nothing about work content, names, identifiers, addresses or free text is included, and the message has no field able to carry any of them. Evidence: [Usage Data](../settings/usagedata.html) |
