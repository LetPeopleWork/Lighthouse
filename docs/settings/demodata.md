...
|---------------|-------------|-----------|------------------|
| When Will This Be Done? | One team with a decent throughput, that works on one portfolio with a set of features. | Basic flow metrics and forecasting demonstrations, onboarding new users, showing Monte Carlo principles | Free |
...
| Dependencies | Features that wait on other Features, and Features several Teams work on. Which dependencies can Lighthouse act on, and which not? | Cross-team dependencies and program management. Shows three kinds of [dependency warning](../features/features.html#warnings) side by side: one Feature waits on a loop, one on a Feature ranked below it, and one on a Feature shared with a team that has closed nothing, which also shows the [Cannot forecast](../concepts/howlighthouseforecasts.html#when-a-team-cannot-be-forecast) state | Premium |
