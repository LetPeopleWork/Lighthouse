...
2. Pick **Delegated permissions** and tick the following scope (exact string, case-sensitive):

    - `vso.work_write` — read and write work items, queries, boards, and area/iteration paths in the Azure DevOps organisation. (You will also see `vso.work` (read-only) and `vso.work_full` (full access including delete) in the same list — Lighthouse needs the read-and-write level.)

    {: .note}
    Lighthouse sends this scope to Microsoft as a **resource-qualified** string: `499b84ac-1321-427f-aa17-267ca6975798/vso.work_write`. The GUID prefix is the Application ID of Microsoft's Azure DevOps service. Under the Microsoft identity platform v2.0 endpoints (which Lighthouse uses), every scope MUST be prefixed with the target resource — otherwise Microsoft defaults the audience to Microsoft Graph and rejects the request with `AADSTS650053: scope 'vso.work_write' that doesn't exist on the resource '00000003-0000-0000-c000-000000000000'` (resource `00000003-…` is Graph). You only need to ensure `vso.work_write` is ticked under **API permissions** in the app registration; Lighthouse handles the resource-prefixing transparently.

    {: .note}
    Lighthouse also requests `offline_access` at the protocol level to obtain a refresh token. You do **not** need to add it as a delegated permission under **API permissions** — it is an OIDC protocol scope, not an Azure DevOps API scope.
...
| `AADSTS70011: invalid scope` or `AADSTS650053: The application … asked for scope 'vso.work_write' that doesn't exist on the resource '00000003-0000-0000-c000-000000000000'` | `vso.work_write` is missing from the app's **API permissions** AND/OR the scope was sent to Microsoft without the Azure DevOps resource prefix (Lighthouse handles the prefixing automatically; this only fires if the permission isn't granted yet). Resource GUID `00000003-…` in the error message is **Microsoft Graph** — the v2.0 endpoints default to Graph as the audience when no resource prefix is supplied. | Re-open the app → **API permissions** → **Add a permission** → **APIs my organization uses** → **Azure DevOps** → **Delegated permissions** → tick `vso.work_write` → **Add permissions** → **Grant admin consent for `<tenant>`** → retry. |
...
| Lighthouse log shows `VssServiceException: Identity {guid} has not been materialized, please use interactive login over the browser first.` on the first API call after connecting (e.g. when loading boards for a new team) | The Entra account you OAuth'd with has never logged into the target Azure DevOps organisation interactively. Azure DevOps maintains its **own** user table separate from Entra ID and refuses to recognise Entra identities until a browser sign-in materialises them. OAuth itself worked; the token is valid; ADO just doesn't know the principal yet. | Open `https://dev.azure.com/{your-org}` (the same Organization URL you put in the Lighthouse connection) in a fresh browser tab, sign in with the **same** Microsoft account you OAuth'd with in Lighthouse, accept any "request access" prompt if you are not yet a member of the org, wait for the ADO home page to load — that one interactive sign-in materialises the identity. Then retry the failing Lighthouse operation. No need to re-do the OAuth consent; the materialisation is independent of the token. This is a one-time-per-user gotcha on first use of the connection — subsequent users of Lighthouse with their own OAuth credentials will hit the same first-sign-in step. |
