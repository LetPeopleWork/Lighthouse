...
- **Rule going forward**: In any new TypeScript browser-facing code, use `globalThis.location.*` rather than `window.location.*`. The same applies to `globalThis.fetch`, `globalThis.document`, etc., where applicable.
