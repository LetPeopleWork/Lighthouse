...
**Static `MetricsCache` is a second root cause, not just the SQLite pool (discovered in Slice-02; scope decision: pull the planned Slice-03 cache fix forward — user-approved 2026-06-15).** Slice-01's triage assigned the metrics read-API fixtures (`*Percentiles*`, `*CumulativeStateTime*`, `*FlowEfficiency*`, `*TimeInState*`, `Named*CycleTime*`, `DeliveryMetricsHistory*`, the blackout/recurring forecast fixtures) to bucket-2 (SQLite pool). Under parallel execution they flaked (`WorkItemAgePercentilesReadApiIntegrationTest` returned an empty population, 1-in-3): the real blocker is `BaseMetricsService`'s process-wide `static Cache<string,object> MetricsCache`, keyed by `{entity.Id}_…`. Two fixtures in *isolated* DBs both seed a team that gets `Id=1`, so they collide on the AppDomain-shared cache key — DB isolation can't fix a process-wide cache. This is the same bucket-3 root cause the triage assigned to Slice-03's `*MetricsServiceTests`. Fix (the canonical DI-not-statics pattern, ci-learnings 2026-05-18): the cache became a per-host DI **singleton** (`AddSingleton<Cache<string,object>>`), resolved via the existing `serviceProvider` field idiom. Behavior-preserving in production (one host = one singleton = identical to the static); each test WAF gets its own instance. `BaseMetricsService.InvalidateMetrics`/`GetMetricFromCache` de-static'd to reach the instance cache.

## Wave: DELIVER / [REF] Slice-02 — integration WAF per-fixture isolation (delivered 2026-06-15)

Implementation summary: each `IntegrationTestBase`-derived fixture now owns its `TestWebApplicationFactory` (shared static `SharedFactoryLazy` removed); SQLite connection pooling defeated per fixture (`Pooling=False`); the process-wide `MetricsCache` made a per-host DI singleton. `[NonParallelizable]` removed from the base + 41 WAF fixtures. The integration suite runs **3× consecutively green** under `ParallelScope.Fixtures`.

Files modified:
- Production: `Services/Implementation/BaseMetricsService.cs` (static cache → per-host DI singleton; 2 cache methods de-static'd), `Program.cs` (`AddSingleton<Cache<string,object>>`).
- Test infra: `TestHelpers/TestWebApplicationFactory.cs` (`Pooling=False`), `TestHelpers/IntegrationTestBase.cs` (per-fixture ownership, drop base `[NonParallelizable]`, expose `WebApplicationFactory`), `TestHelpers/FixtureTimingReporter.cs` (drop shared-factory disposal), `API/Integration/DeliveryMetricSnapshotCascadeDeleteIntegrationTest.cs` (`SharedFactory` → `WebApplicationFactory`).
- Untag (41 fixtures): all `API/Integration/*` self-builders + the 2 intermediate bases' derivers + `S5_ApiKeyScopesTests`, `F_BE_1_GroupSnapshotInheritanceTests`, `LighthouseAppContextConcurrencyTest`, `DeliveryMetricSnapshotRecordingHandlerTest`.
- Test fixes for the cache DI: 4 fixtures constructing a metrics service via `Mock<IServiceProvider>` now return a fresh `Cache` (`TeamMetricsServiceTests`, `TeamMetricsServiceSnapshotTests`, `PortfolioMetricsServiceTests`, `ForecastFilterFeatureForecastIntegrationTest`).

AC check: AC-02.1 ✓ (base un-tagged, per-fixture isolation), AC-02.2 ✓ (3× green: 295/297/283s, 0 failures), AC-02.3 ✓ (behaviour preserved — same 2970 tests pass), AC-02.4 partial (local before 336s → after ~292s/-13%; CI before/after recorded post-push — local was never the bottleneck), AC-02.5 → `BaseMetricsService.cs` added to the feature mutation-gate list (lifetime-only change; run at finalization per review-first convention).
...
## Wave: DELIVER / [REF] Slice-06 — per-fixture WAF reuse, read-API cluster (delivered 2026-06-16)

Implementation summary (test-only): the data-driven read-API integration fixtures built a fresh `TestWebApplicationFactory` in `[SetUp]` (per **test**); moved that to `[OneTimeSetUp]` (per **fixture**) + `[OneTimeTearDown]` disposal, keeping the per-test DB reset (`EnsureDeleted`/`EnsureCreated`) + seeders in `[SetUp]`. **11 fixtures** converted: the PoC `CumulativeStateTimeReadApiIntegrationTest` (f3278c07) + 10 siblings (`CumulativeStateTimePortfolio`, `AgeInStatePercentilesNonLinearFlow`, `AgeInStatePercentiles`, `FlowEfficiency`, `FlowEfficiencyPortfolio`, `WorkItemAgePercentiles`, `WorkItemAgePercentilesPortfolio`, `NamedCycleTimeReadApi`, `NamedCycleTimePortfolio`, `NamedCycleTimeCumulativeScope`).

Cache-isolation safety: these fixtures each give every test a **distinct date window** (`testDateOffset`-derived), and the metrics cache (the per-host DI singleton from Slice-02) is window-keyed — so a reused host across tests can't bleed a stale value even when entity Ids repeat after `EnsureDeleted`. Verified: the 11-fixture cluster green **3× in isolation** (113 tests), and the **full suite green 3×** (324/333/333 s, 2972 passed).

Result: per-fixture, the heaviest fixture's test execution roughly **halved** (~12 s → ~6 s for 25 tests). Full-suite local wall-clock **~345 s → ~330 s (~5%)** — modest on a 12-core box (other long fixtures + total saturation still bound it), but it directly shortens the critical-path serial chains, so the relative gain should be larger on a core-constrained CI runner. Behaviour preserved (same tests/assertions; only WAF lifetime changed).

**Scope call — stopping the rollout here (recommended):** the remaining per-`[SetUp]` builders were deliberately NOT converted:
- **7 mock-injecting builders** (`DeliveryMetricsHistory`, `ForecastFilterThroughputChart`, `ForecastFilterTeamSettings`, `CycleTimeDefinitionSettings`, `CycleTimeDefinitionValidity`, `PortfolioStalenessThresholdSettings`, `TeamStalenessThresholdSettings`) configure the host at build time with per-test mocks (`RemoveAll<ILicenseService>()` + a `Mock<...>`). Reusing the host is only safe if that mock config is test-invariant — a per-fixture judgement with rising risk for a small payoff (these are mostly fast, ≤14 tests).
- **Config-rebuilders** (`S1_AllowedOriginsEnvVarBindingTests`, `S1_CorsFailClosedTests`, `S5_ApiKeyScopesTests`, `F_BE_1_GroupSnapshotInheritanceTests`, the `OAuth*` fixtures) rebuild the WAF on purpose — each test sets a different `Environment` var / auth config read at host startup. Per-test rebuild is load-bearing, not waste.
- **14 small builders** (≤4 tests) save ≤3 host builds each — not worth the churn/risk.

Given #2's measured local gain is modest (~5%), **#3 (in-memory SQLite) is also not pursued** — its ~20 s aggregate `EnsureDeleted`/`Created` cost is a test-fidelity change (DISCUSS-deferred) for marginal return. The feature's avoidable-opt-out and parallel-debt goals (Slices 02–04) are fully met; Slices 05–06 are the bonus work-reduction the user folded in. Recommend finalizing #5258 after the optimization slices' CI run is green.
