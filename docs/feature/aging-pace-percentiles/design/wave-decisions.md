...
| DDD-1 (revised) | Implements the revised D12. Computation lives on the pace path only (`BuildWorkflowStateOrder` + the `BaseMetricsService` cumulative population helper + the two leaf `GetAgeInStatePercentilesFor*` methods). Operates only on normalized `SyncedTransitions` + mapped `DoingStates`; no per-connector branching. **Mapped-state restriction is applied inside `ComputeAgeInStatePercentiles` (filter `doingStatesInWorkflowOrder` to the scope's configured `DoingStates`, preserving order, before iterating); `BuildWorkflowStateOrder` — which today APPENDS observed unmapped exit states — is left untouched.** Empty `DoingStates` (defensive only; a non-empty default ships) falls back to the observed order. Imputation boundaries (reached / last-exit / earliest-at-or-after / tie-break / ≥1-exit invariant) and the no-window-clip rule are specified in ADR-047 §Implementation Notes; the per-item observation is extracted into a `protected static int? CumulativeAgeObservationForItemAtState(...)` helper kept under McCabe 15 (ADR-047 §Consequences), reading only pre-loaded transitions (ArchUnit repo rule preserved). |
| DDD-1b | **Backend non-decreasing safety clamp** — final defensive layer on top of DDD-1. As the FINAL step of `ComputeAgeInStatePercentiles` (after cumulative percentiles computed, states in `doingStatesInWorkflowOrder`), clamp each percentile value to the running max of that SAME rank seen so far (p50/p70/p85/p95 each independent; omitted states skipped, running max carries across). **Placement = backend (API).** Rationale: (1) **single source of truth** — the API guarantees non-decreasing per-rank bands once, not re-implemented per consumer; (2) **FE faithful** — NO FE logic change, the chart renders what the API returns (consistent with the no-FE-change decision); (3) **connector-agnostic** — operates on the normalized, already-ordered output, no `switch (WorkTrackingSystem)`; (4) **contract guarantee** — CLI/MCP and any future API consumer inherit it. No-op under a correct `DoingStates` order (Option A already monotonic); fires ONLY on a misconfigured order (or residual degenerate case). The once-rejected "Option C" clamp is now correct as a SAFETY NET LAYERED ON TOP of the honest Option A data fix, not a substitute (honesty reframing recorded in ADR-047). Does not break the last-column ≈ cycle-time invariant; applied to the same order the chart renders. Pure (no repo, no query); small named helper (`ClampPercentilesNonDecreasing`) under McCabe ≤ 15 / `S3776`. See ADR-047 §Decision → Monotonicity safety clamp + feature-delta DDD-1b. |
| D-5145-A | Option A locked. Option B (mapped-state filter only) folded in; Option C (FE `cummax` clamp) declined *as a standalone FE substitute* — semantically dishonest, manufactures a non-real percentile. The same clamp mechanism is REINSTATED in the **backend** as DDD-1b, a defensive layer on top of the Option A data fix (not a substitute for it). |
...
| D-5145-V | Verification: replace the masking synthetic perfectly-linear integration fixture with non-linear-flow fixtures asserting on the normalized `SyncedTransitions` model; live walking-skeleton uses the CSV / demo-data path carrying non-linear flows (skips / backward / unmapped), plus a real connector if available. **Merge gate (binding, finding-3 close):** (1) the non-linear-flow NUnit integration fixtures are **REQUIRED before merge — they block CI green**; (2) the live walking-skeleton on the CSV / demo-data non-linear path is **REQUIRED before merge** — the demo CSV can express the shape via its `StateEnteredDate_<state>` state-history columns, and per this project's convention **Claude runs this live E2E locally itself (never punted to the user)**; (3) a live real-Jira connector check is **STRONGLY RECOMMENDED but MAY be deferred** to a post-release validation if no Jira sandbox is available during DELIVER — **any deferral MUST be recorded** (DELIVER notes + ADR-047 §Verification) with reason and follow-up owner, never silently skipped. |
