...
| D12 | **Band metric = cumulative total age over the reached-at-least-this-state population** (connector-agnostic). For each mapped `Doing` state `S` (in the team's configured `DoingStates` order), the band's population is **every completed-in-window item that reached `S` or any state at-or-after `S`** in the mapped order; each contributes its **last-exit cumulative age** `= (last exit from S).TransitionedAt − StartedDate`. Items that reached a state at-or-after `S` but have no recorded exit *from* `S` (skip / never-exited / captured only downstream) are **imputed** from the earliest exit they do have at-or-after `S`. Percentiles (50/70/85/95) are computed over those observations via the existing `PercentileCalculator`. Because the population for `S` is a **superset** of the next state's population and every contribution is on the same monotonic age clock, the bands **rise monotonically left→right on every connector** and the rightmost mapped state's band ≈ the cycle-time percentile line — both promises now true **by construction**. The computation operates ONLY on the normalized `WorkItem.SyncedTransitions` model + the team's mapped `DoingStates`; there is **NO per-connector branching** (no `switch (WorkTrackingSystem)`). The non-linear flows that broke the old metric (skips, backward moves, unmapped/intermediate statuses, faster-downstream cohorts) are a data *shape* any source can produce, not a Jira special case. **A backend non-decreasing safety clamp (DDD-1b) is layered on top as a final guarantee against a misconfigured `DoingStates` order — it does NOT change this population/age definition, only adds an API-level monotonicity guarantee.** | **Revised (2026-06-01, bug #5145) — supersedes the 2026-05-25 per-state-exit-population wording**, whose monotonicity + last-column promises were mathematically false on non-linear (any-connector) data. Option B's mapped-state filter is folded in; Option C (FE clamp) declined. See ADR-047 + Changed Assumptions. |
...
|---|---|---|---|
| DDD-1 | Per-state percentile algorithm: **cumulative reached-at-least-this-state population** over the team's mapped `Doing` states. For each mapped state `S`, the population is every completed-in-window item that reached `S` or any state at-or-after `S` (mapped order); each contributes its **last-exit cumulative age** `= (last exit from S).TransitionedAt − StartedDate`; items with no recorded exit from `S` but proof of reaching at-or-after `S` are **imputed** from their earliest at-or-after exit (skip / never-exited). Bucketed by mapped state; percentiles via the existing `PercentileCalculator` (per D12). Operates only on normalized `SyncedTransitions` + mapped `DoingStates` — **no per-connector branching**. **Mapped-state restriction (mechanism, finding-1 close):** the pace path restricts its state set to the scope's mapped `DoingStates` **inside `ComputeAgeInStatePercentiles`, by filtering its `doingStatesInWorkflowOrder` input down to the configured `DoingStates` (preserving order) before iterating** — `BuildWorkflowStateOrder` (which today APPENDS observed unmapped exit states, `orderedStates.AddRange(...)`, lines 31-47) is **left untouched** so the change stays provably pace-local. `DoingStates` ships a non-empty default on `WorkTrackingSystemOptionsOwner`, so it is effectively never empty; if it ever is, the pace path **falls back to the observed order** (filter is a no-op), matching today's behaviour. **Imputation precision (finding-2 close):** "reached `S`" = `Any(t.FromState == S \|\| t.ToState == S)`; "last exit from `S`" = `max(TransitionedAt where FromState == S)`; "earliest at-or-after exit" = `min(TransitionedAt where FromState is a mapped Doing state ordered ≥ S)`; tie-break = smallest `TransitionedAt`. `WorkItemStateTransition.FromState` is non-nullable `string` (default `string.Empty`), so an exit is `!string.IsNullOrEmpty(FromState)`; a completed item always has ≥1 such exit, so imputation always finds a candidate (assert as a test invariant). Cumulative age is **not** clipped to the window (runs from `StartedDate`, may include pre-window transitions, mirroring `cycleTimePercentiles`). Full definitions in ADR-047 §Implementation Notes. | **Revised (2026-06-01, bug #5145) — supersedes the 2026-05-25 visit-level per-state-exit formula**, whose population differed per state on non-linear data, breaking monotonicity + last-column alignment. Superset population on a single monotonic clock makes both true by construction, on every connector. See ADR-047 §Implementation Notes |
| DDD-1b | **Backend non-decreasing safety clamp** (final defensive layer on top of DDD-1). Option A (DDD-1) is genuinely monotonic **only given a correct user-configured `DoingStates` order**; a misconfigured order (a logically-left state placed right in config) could still produce a downward band. This is NOT the cause of bug #5145's screenshots, but it is a residual path. **Mechanism:** as the FINAL step of `ComputeAgeInStatePercentiles`, AFTER the cumulative reached-at-least-this-state percentiles are computed and the states are in `doingStatesInWorkflowOrder`, walk the ordered states and clamp each percentile value to the running maximum of that SAME percentile rank seen so far (p50→max prior p50, p70→p70, p85→p85, p95→p95 — each rank independent); a state's clamped value `= max(itsTrueValue, runningMaxForThatRank)`. Omitted states (no observations, DDD-4) are skipped and the running max carries across the gap. **Placement = backend (API), not FE:** the API itself GUARANTEES non-decreasing per-rank bands across the returned workflow-ordered states; the FE stays faithful (**NO FE logic change**, consistent with the no-FE-change decision); CLI/MCP and any future API consumer inherit the guarantee at the single source of truth. Connector-agnostic (operates on the normalized, already-ordered output; no `switch (WorkTrackingSystem)`). **No-op under correct config:** Option A already yields monotonic values, so the clamp never adjusts anything — it only ever raises values, only in the misconfigured / residual-degenerate case. **Honesty reframing:** this is the same clamp previously DECLINED as "Option C — dishonest"; its role is now a SAFETY NET LAYERED ON TOP of the honest Option A data fix, not a substitute. Option-C-alone masked the real per-state-population flaw with fabricated values across the board; with Option A fixing that flaw, the clamp fires ONLY on a misconfigured order (an input error where the "true" per-state value is itself meaningless), so flattening a drop to the previous level is the lesser evil vs. rendering a confusing dip. **Last-column invariant preserved:** the rightmost column is the largest in a monotonic series, so the running-max clamp leaves it unchanged in the normal case; in a misconfigured case alignment is already moot. The clamp is applied to the SAME workflow order the chart renders, so backend and chart agree. **Pure** (no repository, no query — operates on already-computed ordered DTOs; ArchUnit repo rule preserved); extract a small named helper (e.g. `ClampPercentilesNonDecreasing`) to keep McCabe ≤ 15 / `S3776`. Full definition in ADR-047 §Decision → Monotonicity safety clamp + §Implementation Notes + §Consequences. | **Added (2026-06-01, bug #5145)** as a distinct guarantee layered on DDD-1; does NOT change the DDD-1 population/age definition. | Defensive guarantee against a misconfigured `DoingStates` order; the once-rejected clamp is now correct as a defensive layer on top of the honest data fix; ADR-047 |
...
|---|---|---|
| DDD-1 | **Cumulative reached-at-least-this-state population** over mapped `Doing` states: per mapped state `S`, population = items that reached `S` or beyond; observation = last-exit cumulative age (`(last exit from S).TransitionedAt − StartedDate`); skip/never-exited imputed from earliest at-or-after exit; mapped-state filter folded in; operates only on normalized `SyncedTransitions` + mapped `DoingStates`, no per-connector branching (revised 2026-06-01, bug #5145, supersedes the per-state-exit framing) | ADR-047 (supersedes ADR-019 §1/§3) |
...
## Wave: DESIGN / [REF] Bug #5145 metric redesign

ADO: https://dev.azure.com/letpeoplework/Lighthouse/_workitems/edit/5145 — revised 2026-06-01, interaction mode PROPOSE. ADR pointer: **[ADR-047](../../product/architecture/adr-047-aging-pace-cumulative-population.md)**.

**Mechanism.** Replace the per-state-exit population (ADR-019 D12/DDD-1, shipped) with a **cumulative reached-at-least-this-state population** over the team's mapped `Doing` states. Per mapped state `S` (in `DoingStates` order): population = every completed-in-window item that reached `S` or any state at-or-after `S`; each item contributes its **last-exit cumulative age** `= (last exit from S).TransitionedAt − StartedDate`; items proven to have reached at-or-after `S` but with no recorded exit *from* `S` (skip / never-exited / captured only downstream) are **imputed** from their earliest at-or-after exit. Percentiles, window membership (`ClosedDate ∈ window`), day-counting, and caching are unchanged from ADR-019. Because each state's population is a superset of the next state's and every contribution is on one monotonic age clock, the bands rise monotonically left→right and the rightmost band ≈ the cycle-time line — **by construction**, no longer by fixture luck. Option B's mapped-state filter is folded in; Option C (FE `cummax` clamp) declined as semantically dishonest.

**Connector-agnostic constraint.** The computation reads ONLY the normalized `WorkItem.SyncedTransitions` model (every connector emits the same `(FromState, ToState, TransitionedAt)` triples after its own mapping) and the team's configured mapped `DoingStates`. There is **NO per-connector branching** — no `switch (WorkTrackingSystem)`, no Jira/ADO/Linear/CSV special case. The non-linear flows that broke the old metric (skips, backward moves, unmapped/intermediate statuses, faster-downstream cohorts) are a data *shape* any source can produce, not a Jira special case; Jira just surfaces it more often. A future connector inherits correct bands the moment it populates the normalized transition model, zero code change.

**Blast radius — pace path only.** `BuildWorkflowStateOrder` is **pace-exclusive**; the cumulative population walk lives in the `BaseMetricsService` pace helper + the two leaf `GetAgeInStatePercentilesFor*` methods. The sibling **cumulative-state-time chart is provably untouched** (different membership rule; never calls `BuildWorkflowStateOrder` or the pace walk; ADR-024 cross-call reflection rule preserved). The mapped-state filter is scoped to the pace path and touches no other metric.

**No schema / contract / cache-key change.** DTO (`AgeInStatePercentilesDto`), endpoints, FE model, and response shape are all unchanged — only the *values* differ. No EF migration. The cache key stays `AgeInStatePercentiles_{startDate:yyyy-MM-dd}_{endDate:yyyy-MM-dd}`: the in-process static metric cache is dropped on every deploy (restart-on-deploy) and the post-sync `InvalidateMetrics` hook recomputes per team/portfolio, so no stale value survives rollout and a cache-key bump would be dead ceremony (recorded to forestall a redundant future bump).

**Verification (specified; fixtures/impl are DELIVER's).** The masking synthetic perfectly-linear integration fixture is **replaced** by non-linear-flow fixtures that assert behavior on the **normalized `SyncedTransitions` model** (not a Jira-specific payload) — items that skip a mapped state, move backward, traverse unmapped statuses, and form faster-downstream cohorts. The live walking-skeleton check drives the **CSV / demo-data path** (which every connector feeds into the same normalized model) carrying **non-linear flows**, plus a real connector if available; the demo CSV already carries state history via `StateEnteredDate_<state>` columns and MUST be able to express skips / backward-moves / unmapped states so the live check exercises the non-linear shape. ArchUnit guards that no `WorkTrackingSystem` switch leaked into the pace walk and that transitions are still read only via the repositories.

## Changed Assumptions

**Source**: `docs/feature/aging-pace-percentiles/feature-delta.md` → D12 (DISCUSS locked-decisions table), as locked 2026-05-25.

**Original assumption (verbatim, the 2026-05-25 per-state-exit-population wording)**:
> **Band metric = cumulative total age at state exit** (NOT time-spent-in-that-state). For each completed item and each `Doing`-category state it passed through, the observation is the item's **total work-item age at the moment it left that state** = `exitTransition.TransitionedAt − StartedDate` (the same age units the chart's Y axis already uses for the in-flight dots). Percentiles are computed over those total-age-at-exit observations, bucketed by state. Because total age accumulates as work moves right through the workflow, the bands **rise monotonically left→right** — matching ActionableAgile and making each band directly comparable to the dots plotted above its column.

**New assumption (revised D12 / DDD-1, 2026-06-01)**:
Per mapped `Doing` state `S`, percentiles are computed over the **cumulative reached-at-least-this-state population** — every completed-in-window item that reached `S` or any state at-or-after `S` — with each item contributing its **last-exit cumulative age** and skip / never-exited items imputed from their earliest at-or-after exit. The mapped-state filter (Option B) is folded in. The computation operates only on the normalized `SyncedTransitions` model + the team's mapped `DoingStates`, with no per-connector branching.

**Rationale**: bug #5145's RCA established that the original D12's two promises — **monotonicity** (bands rise left→right) and **last-column alignment** (rightmost band ≈ the cycle-time line) — are **mathematically false on non-linear (any-connector) data**. The per-state-exit *population* differs per state: a state that downstream items skip or reach faster has a systematically faster cohort than an upstream state whose population still includes the slow items that never reached downstream, so an upstream band can sit *above* a downstream one, and the rightmost mapped state is reached only by the items that got that far so its band does not align with end-to-end cycle time. The original integration fixture was a synthetic **perfectly-linear** flow on which both cohorts coincide and the promises held *vacuously* — it masked the defect. The revised metric makes each state's population a **superset** of the next state's on one monotonic age clock, so both promises are true **by construction**, on every connector. **Option B** (mapped-state filter only) is insufficient alone (residual non-monotonicity among purely mapped states from faster-downstream cohorts) but its filter is correct and is **folded in**. **Option C** (FE `cummax` clamp) is **declined** — a clamped band is no longer a real percentile of any population, i.e. it lies to the user; it was justified only as an interim stopgap, and Option A is a contained pace-path change needing no interim.

## Wave: DISTILL / [REF] Bug #5145 metric redesign — scenario list with tags

Acceptance tests for the Option A redesign are authored as RED scaffolds against the **HTTP driving port** `GET /api/latest/teams/{teamId}/metrics/ageInStatePercentiles`, in a sibling fixture so the existing clean-data fixture's GREEN tests stay untouched. Each RED scaffold carries `[Ignore("RED scaffold — pending #5145 Option A redesign; DELIVER un-ignores one at a time")]` (project green-before-push ritual); DELIVER un-ignores one at a time as it implements the cumulative population + imputation. Layer 3 (real WebApplicationFactory + EF InMemory) ⇒ example-only, no PBT (Mandate 9/11). Sad/non-linear paths are explicit named examples (Mandate 11).

| # | Scenario (NUnit test) | Tags | Intent (RED on current → GREEN under Option A) |
|---|---|---|---|
| 1 | `GetAgeInStatePercentiles_OldMetricNonMonotonicOnFasterDownstreamCohort_OptionACorrectsToRiseMonotonicallyAcrossEveryConnector` (`[TestCase]` × AzureDevOps, Jira, Linear) | `@real-io` `@driving_port` `@US-01` `@property(monotonicity)` `@connector-agnostic` `@rca-regression` | **RCA-regression fixture** (named + `[Ignore]` reason call out the #5145 RCA): a slow fully-ordered cohort + a fast cohort that **skips Review** is the exact shape the OLD per-state-exit metric renders non-monotonic (Test p < Review p); Option A corrects it. Asserts 50/70/85/95 rise `In Progress ≤ Review ≤ Test`. Same seed path across all three connectors proves no per-connector branching. Single WHY comment on the seed records the old-vs-new contrast. |
| 2 | `GetAgeInStatePercentiles_AllItemsCloseFromLastMappedState_LastStateMatchesCycleTimePercentilesAcrossEveryConnector` (`[TestCase]` × AzureDevOps, Jira, Linear) | `@real-io` `@driving_port` `@US-01` `@property(last-column-alignment)` `@connector-agnostic` | Items rework on the last state (`Test→In Progress→Test→Done`). Current double-counts the early Test exit, dragging the last band away from the cycle-time line. Asserts last state's 50/70/85/95 equal the `cycleTimePercentiles` endpoint over the same window. **Now parametrized across all three connectors** (ADR-047 mandates the last-column invariant on every connector) — same seed path threaded with the system parameter, matching test #1. |
| 3 | `GetAgeInStatePercentiles_ItemsSkipReview_ReviewPopulationIncludesSkippersByImputation` | `@real-io` `@driving_port` `@US-01` | A `In Progress → Test` skipper cohort (imputed Review ages **{12,13,14,15}**) alongside `In Progress → Review → Test` visitors (Review ages **{2,3,4,5}**). Every skipper age exceeds every visitor age, so the with-imputation Review p70 lands at **12** — a value reachable ONLY from the imputed set; the visitors-only population {2,3,4,5} cannot yield 12 at any percentile, making the assertion uniquely diagnostic (the DTO has no count field, so p70=12 is the only observable signal that imputation ran). Also asserts Review is **present** in the returned states (skippers must not omit it). Single WHY comment on the seed shows the percentile math. |
| 4 | `GetAgeInStatePercentiles_ReworkItemReentersReview_ReviewRecordsOnlyTheLastExit` | `@real-io` `@driving_port` `@US-01` | One item: `In Progress → Review → In Progress → Review → Test`. Current records BOTH Review exits (p50=5 from the first). Asserts every Review percentile = 15 (the LAST exit cumulative age, one observation per item per state). |
| 5 | `GetAgeInStatePercentiles_UnmappedStatusInTransitions_IsExcludedFromPacePath` | `@real-io` `@driving_port` `@US-01` `@infrastructure-failure(unmapped-status)` | Team `DoingStates=[In Progress, Review, Test]` with an injected `Waiting for feedback` (unmapped) exit. Current `BuildWorkflowStateOrder` appends it as an observed exit so it leaks into the response. Asserts the pace response does **not** contain `Waiting for feedback` while mapped states remain. |
| 6 | `GetCumulativeStateTime_ForTeamWithUnmappedStatus_IsUnaffectedByThePacePathFilter` | `@real-io` `@driving_port` `@characterization` `@no-regression` | Sibling-chart guard (GREEN now, NOT a RED scaffold). Calls the `cumulativeStateTime` endpoint for the same unmapped-status team; asserts state order `[In Progress, Review, Test]` — the cumulative chart uses the separate `BuildCumulativeWorkflowStateOrder` and is provably untouched by the pace-path mapped-state filter. |
| 7 | `GetAgeInStatePercentiles_MisconfiguredDoingStatesOrder_BandsNeverDropBelowThePreviousState` | `@real-io` `@driving_port` `@US-01` `@property(non-decreasing-clamp)` `@DDD-1b` | **DDD-1b non-decreasing safety clamp.** Team `DoingStates` configured in a WRONG order `[Review, In Progress, Test]` (In Progress placed right of Review), seeded so the true per-state cumulative-age values DIP along that configured order (`Review=21, In Progress=11, Test=31`). Asserts (a) states return in the configured order, (b) every rank 50/70/85/95 is non-decreasing across the returned states, and (c) **uniquely diagnostic**: the dipping In Progress value comes back EQUAL to (clamped to) the preceding Review value — `11` unclamped vs `21` clamped — so the test cannot pass vacuously on unclamped output. Single WHY comment on the seed records the misconfigured order + pre-clamp dip vs clamped-equal expectation. |

**FE characterization guard** (GREEN now, NOT a RED scaffold): `WorkItemAgingChart.test.tsx` → `collapses zero-height bands and keeps the surviving 50-70 band green when upper percentiles coincide` already locks that `computePaceBandRects` keys color off band **position**, not percentile value — so when adjacent percentiles coincide the zero-height band drops and no surviving band shifts color. A single WHY comment was added documenting that this locks "the FE needs no change under Option A". No duplicate test added (DRY).

## Wave: DISTILL / [REF] Bug #5145 metric redesign — RED-for-the-right-reason classification

Each RED scaffold was run once **un-ignored** through the HTTP endpoint against current production code; all failed as **assertion failures** (`MISSING_FUNCTIONALITY`), never compile/setup/import errors — the new tests compile and run against current types (no not-yet-existing types referenced; the DTO has no `observationCount`/`sampleSize` field per DDD-4, so observation-count claims are asserted indirectly through observable percentile values).

| # | Scenario | Classification | Observed RED (current) | Target GREEN (Option A) |
|---|---|---|---|---|
| 1 | OldMetricNonMonotonicOnFasterDownstreamCohort (×3 connectors) | `MISSING_FUNCTIONALITY` ✅ | `Test p50 = 7` vs `Review p50 = 40` → `Test ≥ Review` fails (non-monotonic); identical on AzureDevOps/Jira/Linear | monotonic rise on all connectors |
| 2 | LastStateMatchesCycleTime (×3 connectors) | `MISSING_FUNCTIONALITY` ✅ | last-state pace `p50=13/p70=16/p85=20` vs cycle-time `16/20/22`; identical on AzureDevOps/Jira/Linear | last state == cycle-time line on every connector |
| 3 | ItemsSkipReview imputation | `MISSING_FUNCTIONALITY` ✅ | `Review p70 = 5` (visitors-only population {2,3,4,5}) vs expected `12` (with imputed skippers {12,13,14,15}) — 12 is unreachable from the visitor set, so the failure uniquely proves imputation is missing | skippers imputed into Review; p70 lands on the imputed-only value 12 |
| 4 | ReworkItemReentersReview | `MISSING_FUNCTIONALITY` ✅ | `Review p50 = 5` (first exit kept) vs expected `15` on all percentiles | one observation per item = last exit |
| 5 | UnmappedStatusExcluded | `MISSING_FUNCTIONALITY` ✅ | response states `[In Progress, Review, Test, Waiting for feedback]` → `Does.Not.Contain` fails | unmapped status excluded |
| 6 | CumulativeStateTime guard | `PASS` (characterization) ✅ | already GREEN — cumulative chart unaffected | stays GREEN |
| 7 | MisconfiguredDoingStatesOrder non-decreasing clamp (DDD-1b) | `MISSING_FUNCTIONALITY` ✅ | states return in configured order `[Review, In Progress, Test]` (order assertion already passes), but values dip `Review=21, In Progress=11, Test=31` → `In Progress ≥ Review` fails (`11` vs `21`); current code applies NO clamp, so the dipping value comes back as its true `11`, not the clamped `21` | clamp raises In Progress to EXACTLY the preceding Review value (`21`); all ranks non-decreasing across the returned states |

DELIVER reads this table at its RED-phase entry gate to confirm RED is genuine before un-ignoring each scaffold.

## Wave: DISTILL / [REF] Bug #5145 metric redesign — test placement + DELIVER live-check note

- **Backend (new)**: `Lighthouse.Backend/Lighthouse.Backend.Tests/API/Integration/AgeInStatePercentilesCumulativePopulationReadApiIntegrationTest.cs` — sibling to the existing `AgeInStatePercentilesReadApiIntegrationTest.cs`. Sibling (not extension) keeps the existing clean-data GREEN tests visibly untouched and groups the #5145 redesign scaffolds. `[NonParallelizable]` fixture (builds `WebApplicationFactory` + EF InMemory, mirrors the existing fixture). New helper variant `AddTeamWithDoingStates(sp, system, doingStates)` seeds `team.DoingStates` so the mapped-state filter is provable; all other helpers (`AddCompletedItem`, `AddExitTransition`, JSON readers) mirror the existing fixture. Tests #1 and #2 are `[TestCase]`-parametrized across `AzureDevOps`/`Jira`/`Linear` (ADR-047: the monotonicity and last-column-alignment invariants must hold on every connector) — each threads the `WorkTrackingSystems` parameter through its seed (`SeedFasterDownstreamCohort(system)`, `SeedAllItemsClosingFromLastMappedState(system)`). The fixture carries exactly **two** single-line WHY comments (the only comments allowed): one on `SeedFasterDownstreamCohort` recording the old-vs-new monotonicity contrast (test #1, RCA-regression), one on `SeedReviewSkippersAlongsideReviewVisitors` showing the imputation percentile math that makes test #3's `p70 == 12` uniquely diagnostic.
- **Frontend (existing, annotated)**: `Lighthouse.Frontend/src/components/Common/Charts/WorkItemAgingChart.test.tsx` — one WHY comment added to the pre-existing degenerate-band characterization test. No new test (the exact characterization already exists and is green).
- **Existing clean-data tests stay green**: confirmed by a filtered run — `…ReturnsExactCumulativeAgeAtExitPercentilesPerState`, `…BandValuesRiseAcrossStatesInWorkflowOrder`, the `ComputeAgeInStatePercentiles_*` unit tests, the portfolio tests, and the ArchUnit seam tests all PASS. Clean fully-ordered data is identical under old and new metric (reached-at-least = all items, last-exit = the only exit), so these are correctly unaffected; the new non-linear monotonicity test (#1) is now the authoritative monotonicity guard.
- **DELIVER live-check (NOT authored now)**: a full live walking-skeleton on the **CSV / demo-data path** carrying non-linear flows (skips, backward-moves, unmapped statuses) is a DELIVER-time check — EF InMemory misses data-shape regressions per ADR-047 §Verification. The demo CSV must be able to express the non-linear shape via its `StateEnteredDate_<state>` columns. No Playwright spec authored at DISTILL (project E2E-minimalism: implementation-invariant assertions belong in backend integration, which these are).

## Wave: DISTILL / [REF] Bug #5145 metric redesign — reconciliation + ci-learnings

- **Wave-Decision Reconciliation**: DISCUSS D12 (revised) ↔ DESIGN DDD-1 (revised) ↔ ADR-047 are mutually consistent (cumulative reached-at-least population, connector-agnostic, pace-path-only, no schema/contract change). **0 contradictions** — gate passed.
- **ci-learnings pre-applied**: `Is.GreaterThanOrEqualTo` / `Is.Zero` style; no ≥2 consecutive bare `Assert.That` outside `Assert.EnterMultipleScope`; factory-helper test data, no shared mutable state; `[NonParallelizable]` on the fixture; zero banned comments. Three WHY comments are the only comments present and are the allowed exception: one on the FE characterization guard, and two on the BE fixture seeds (`SeedFasterDownstreamCohort` old-vs-new monotonicity contrast for the test #1 RCA-regression; `SeedReviewSkippersAlongsideReviewVisitors` imputation percentile math for test #3's uniquely-diagnostic `p70 == 12`). `dotnet build` of the test project: **0 warnings, 0 errors**; filtered run green (1 characterization guard passed, 9 RED scaffolds skipped — the 3-connector parametrization of tests #1 and #2 shows as 6 of those skips).
