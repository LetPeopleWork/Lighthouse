...
- **Atlassian Agile API (`/rest/agile/...`) returns 401 "scope does not match" against `api.atlassian.com/ex/jira/{cloudId}/`** even after the OAuth handshake is fully successful and the token explicitly carries `read:board-scope:jira-software`, `read:sprint:jira-software`, and `read:issue:jira-software` alongside the classic `read:jira-work` / `read:jira-user`. The credential is persisted with the granted scopes (confirmed via the new `jira.oauth.token granted scope from Atlassian: …` log line); outbound calls successfully hit the gateway URL; Atlassian rejects with the generic message regardless. Plain Jira platform endpoints (`/rest/api/2/myself` etc.) work with the same token. Open question with Atlassian support — until they reply, **the board picker on team-creation will return zero results for `jira.oauth` connections** even though the connection's status shows Connected. Workaround for now: use `jira.cloud` (API token) or `jira.scopedtoken` if board-driven team setup is required. Tracked: this limitation is documented; no Lighthouse-side fix is known.
- **Pre-save validation is skipped for OAuth in the create wizard.** Validate-on-Next runs only the OAuth handshake; the Jira `/rest/api/2/myself` probe is bypassed because `EnsureFreshTokenAsync` is purposefully minimal in this slice (returns the stored access token, no expiry-aware refresh — that lands in slice 02 per US-02). The probe would otherwise crash the validate path. Connection sync at runtime uses the stored access token directly.
- **Edit-page locks Client ID / Client Secret / Jira URL.** Rotating the OAuth app's credentials requires creating a new connection through the wizard; in-place edit is intentionally disabled to avoid drift between displayed and actually-sent values.
- **Abandoned OAuth drafts.** If the user closes the browser tab during the Atlassian consent step (after Next has saved the draft connection), the draft persists in the DB with no `OAuthCredential` row. The wizard's Cancel button cleans up if clicked; tab-close abandonment does not. A background sweep is out of scope for this slice.
