...
- **Opt-in switch, decided 2026-09-24, part of THIS story (no separate ADO item) = slice 05.** DISCUSS
  amendment done (`37a490652`, `428f6e1f8`: D9, US-05, `slices/slice-05-opt-in-switch.md`). Gate the fill
  in `OverTimeGapReconciler` behind a new instance-wide optional feature, seeded OFF, `IsPreview = true`,
  System Admin toggles it (existing guard). **Backend-only**: the widgets never learn the mode — ONE empty
  sentence in both modes, "Nothing to show for the selected range. Days appear here as Lighthouse records
  them.", replacing the 04-02 sentence (widget tests + E2E constants change with it). Demo data does NOT
  switch it on; E2E/screenshot fixtures turn it on explicitly. Not gated: the recorder no longer writing
  all-zero rows, the as-of threading, the shared writers. Lighthouse-Clients calls both endpoints and its
  copy says they "never backfill" — rewrite at finalization (DoD 14). Next: DESIGN amendment (ADR-207;
  per-request switch read cost vs "no extra query on the read path"; does a running fill stop when the
  switch goes off) → DEVOPS amendment → DISTILL off-state ATs + fixtures on → DELIVER slice 05, all
  before 04-03 docs. Follow-ups in ADO, unparented: #6083 default on after first positive feedback, #6084
  remove the switch once it stays.
