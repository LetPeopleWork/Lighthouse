...
| **OC-4** | Does #5610's picker pre-fill the label or the class name? | **Open, deliberately — maintainer, 2026-08-01: check back once #5610 is delivered.** The right answer is the label — and once DD-4 was deleted this stopped being cosmetic: nothing normalises a class name afterwards, so a picker-configured team reads `change_request` in its Type column forever. Trigger: #5610 finishing DELIVER on `main`. |
