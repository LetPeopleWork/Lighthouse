...
export default defineConfig({
	test: {
		...
		// Stryker runs the whole suite per mutant; sweeping all 291 files OOMs the node heap,
		// so the mutation run sees only the specs covering the mutated files (Bug #5628 precedent).
		include: [
			"src/components/Common/Charts/ChartLegend.test.tsx",
			"src/components/Common/Charts/LegendChip.test.tsx",
			"src/components/Common/Charts/DeliveryEpicSizeChart.test.tsx",
			"src/components/Common/Charts/DeliveryFeverChart.test.tsx",
			"src/components/Common/Charts/deliveryEpicColors.test.ts",
