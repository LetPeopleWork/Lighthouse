...
- New entries in `widgetInfoMetadata.ts` (`stateTimeCumulative` description + status guidance) and `categoryMetadata.ts` (place in `flow-metrics`, size `large`, no `ownerFilter`)
- New `computeCumulativeStateTimeRag` function in `ragRules.ts` with 40% / 60% thresholds per AC
...
- US-03 tooltip enrichment with partial/full attribution counts (slice 02)
...
- **Confirms if it succeeds**: that D5's clip-to-window arithmetic is well-defined for the edge cases (item entering before window, item exiting after window, item still in state at window end) AND that the chart's bar-stacking visual reads correctly to a user reviewing the screenshot.
