...
| DDD-6 | Counts for US-03 tooltip: `completedItemCount[S] = distinct included items with ≥1 completed visit through S`; `ongoingItemCount[S] = included items currently in S`; `itemCount[S] = distinct contributing items` (note: completed + ongoing may exceed itemCount because re-entered items can be in both buckets) | Locked | Implements US-03 inclusion-breakdown line; the non-additive case is documented via tooltip wording ("A closed in window AND B still in flight"); ADR-022 §6 |
...
| DDD-17 | Auth: existing `[RbacGuard(TeamRead, ScopeIdRouteKey="teamId")]` / `[RbacGuard(PortfolioRead, ScopeIdRouteKey="portfolioId")]` class-level guards inherited by all four new endpoints; no new auth surface | Locked | Per CLAUDE.md "no component fetches RBAC summaries directly"; inherits established controller-level guard pattern; DESIGN mechanical |
...
| `ITeamMetricsService` | `Lighthouse.Backend/Lighthouse.Backend/Services/Interfaces/ITeamMetricsService.cs` | EXTEND | Add two methods: `CumulativeStateTimeDto GetCumulativeStateTimeForTeam(Team team, DateTime startDate, DateTime endDate)`; `CumulativeStateTimeItemsDto GetCumulativeStateTimeItemsForTeam(Team team, string state, DateTime startDate, DateTime endDate)`. |
| `IPortfolioMetricsService` | `Lighthouse.Backend/Lighthouse.Backend/Services/Interfaces/IPortfolioMetricsService.cs` | EXTEND | Same shape as Team — two new methods for the portfolio scope. |
| `BaseMetricsService` | `Lighthouse.Backend/Lighthouse.Backend/Services/Implementation/BaseMetricsService.cs` | EXTEND | Add two `protected` helpers: `ComputeCumulativeStateTime(IEnumerable<WorkItem> includedItems, Func<int, IEnumerable<WorkItemStateTransition>> getTransitionsForItem, IEnumerable<string> workflowStatesInOrder, DateTime nowSnapshot)` returning `CumulativeStateTimeDto`; `ComputeCumulativeStateTimeItems(IEnumerable<WorkItem> includedItems, Func<int, IEnumerable<WorkItemStateTransition>> getTransitionsForItem, string selectedState, DateTime nowSnapshot)` returning `CumulativeStateTimeItemsDto`. Per ADR-022 algorithm. INTRA-INHERITANCE only (ADR-024). |
| `TeamMetricsService` | `Lighthouse.Backend/Lighthouse.Backend/Services/Implementation/TeamMetricsService.cs` | EXTEND | Implement `GetCumulativeStateTimeForTeam` by: (1) resolve D12 candidate items via `workItemRepository.GetAllByPredicate` + transition-intersection check; (2) build the transition-fetcher delegate `id => transitionRepo.GetAllByPredicate(t => t.WorkItemId == id)` (or an EF-friendly batch shape — software-crafter at GREEN); (3) call `ComputeCumulativeStateTime(...)`; (4) wrap in `GetFromCacheIfExists(team, $"CumulativeStateTime_{startDate:yyyy-MM-dd}_{endDate:yyyy-MM-dd}", () => …, logger)`. Implement `GetCumulativeStateTimeItemsForTeam` similarly with cache key `CumulativeStateTime_Items_{state}_{startDate}_{endDate}`. Workflow state order from the team's existing `WorkflowStates`. |
| `PortfolioMetricsService` | `Lighthouse.Backend/Lighthouse.Backend/Services/Implementation/PortfolioMetricsService.cs` | EXTEND | Same pattern as Team, scoped to the portfolio's items. |
| `TeamMetricsController` | `Lighthouse.Backend/Lighthouse.Backend/API/TeamMetricsController.cs` | EXTEND | Add `[HttpGet("cumulativeStateTime")] ActionResult<CumulativeStateTimeDto> GetCumulativeStateTimeForTeam(int teamId, [FromQuery] DateTime startDate, [FromQuery] DateTime endDate)` with validation mirroring `cycleTimePercentiles` (HTTP 400 on inverted dates, `LogDateBoundaries`, `GetEntityByIdAnExecuteAction`). Add `[HttpGet("cumulativeStateTime/items")] ActionResult<CumulativeStateTimeItemsDto> GetCumulativeStateTimeItemsForTeam(int teamId, [FromQuery] string state, [FromQuery] DateTime startDate, [FromQuery] DateTime endDate)` with additional `state` presence validation (HTTP 400 on empty/missing). |
| `PortfolioMetricsController` | `Lighthouse.Backend/Lighthouse.Backend/API/PortfolioMetricsController.cs` | EXTEND | Mirror endpoints for portfolio scope. |
...
| `MetricsService` (TS) + `IMetricsService` | `Lighthouse.Frontend/src/services/Api/MetricsService.ts` | EXTEND | Add four methods: `getCumulativeStateTimeForTeam(teamId, startDate, endDate): Promise<ICumulativeStateTimeResponse>`; `getCumulativeStateTimeForPortfolio(portfolioId, startDate, endDate)`; `getCumulativeStateTimeItemsForTeam(teamId, state, startDate, endDate): Promise<ICumulativeStateTimeItemsResponse>`; `getCumulativeStateTimeItemsForPortfolio(portfolioId, state, startDate, endDate)`. All four mirror `getCycleTimePercentiles` HTTP shape (line 247). Added to the `IMetricsService` interface. |
| `useMetricsData` (hook) | `Lighthouse.Frontend/src/hooks/useMetricsData.ts` | EXTEND | Add `cumulativeStateTime: ICumulativeStateTimeResponse | null` (initial `null`) to the returned ctx. In the same `useEffect` block that today fetches `cycleTimePercentiles`, add a parallel call to `metricsService.getCumulativeStateTime…(entity.id, startDate, endDate)`. Both calls share the same window state and run in parallel via `Promise.all` (software-crafter chooses shape at GREEN). The drill-down items fetch is NOT in this hook — it's lazy, fetched on bar click by the chart-parent (BaseMetricsView). |
| `BaseMetricsView` | `Lighthouse.Frontend/src/pages/Common/MetricsView/BaseMetricsView.tsx` | EXTEND | Add a dispatch entry for `widgetKey === "stateTimeCumulative"` that renders `<CumulativeStateTimeChart states={ctx.cumulativeStateTime?.states ?? []} isLoading={ctx.isLoading} onBarClick={handleBarClick} />`. Add local state for the drill-down dialog (selected state name + resolved items + open flag). `handleBarClick(stateName)` fetches via `metricsService.getCumulativeStateTimeItems…` and opens `<CumulativeStateTimeDrillDownDialog>`. |
| `CumulativeStateTimeChart` | `Lighthouse.Frontend/src/components/Common/Charts/CumulativeStateTimeChart.tsx` | NEW | MUI-X `<BarChart>` with `layout="horizontal"` (or vertical — software-crafter picks at GREEN), stacked completed/ongoing series, SVG `<defs><pattern id="cumulativeStateTimeOngoingHatch" .../></defs>` for hatching, `<ChartsTooltip>` custom slot rendering all 10 fields per ADR-025 §2. Bar `onClick` fires `props.onBarClick(stateName)`. Empty case + zero-contributing-state placeholder per AC. |
| `CumulativeStateTimeDrillDownDialog` | `Lighthouse.Frontend/src/components/Common/Charts/CumulativeStateTimeDrillDownDialog.tsx` | NEW | MUI `Dialog` + `DialogTitle` + `DialogContent` wrapping `DataGridBase` with columns (Work Item ID linkable, Title, Type, Current State, Days Contributed). Default sort: `daysContributed` desc. ARIA `role="dialog"`, focus trap, Escape closes, outside-click closes. Empty case: "No items contributed to this state in the selected window." per US-04 AC. |
...
| Vitest tests — `CumulativeStateTimeChart` | `Lighthouse.Frontend/src/components/Common/Charts/CumulativeStateTimeChart.test.tsx` | NEW | (a) Renders one bar per state in workflow order; (b) bar heights match completedContributionDays + ongoingContributionDays; (c) tooltip on hover contains all 10 fields + US-03 inclusion-breakdown line + full-duration attribution clarification; (d) hatching pattern element present in DOM; (e) bar click fires onBarClick with state name; (f) empty case renders empty-state message; (g) zero-contributing state renders labelled placeholder. |
...
| NUnit tests — `TeamMetricsService.GetCumulativeStateTimeForTeam` + items variant | `Lighthouse.Backend/Lighthouse.Backend.Tests/Services/Implementation/TeamMetricsServiceTests.cs` | EXTEND | D12 inclusion-rule integration test against EF InMemory fixture: assert candidate set matches the expected union. Cache-key shape test: assert key passed to `GetFromCacheIfExists` matches `CumulativeStateTime_{startDate:yyyy-MM-dd}_{endDate:yyyy-MM-dd}` / `CumulativeStateTime_Items_{state}_{startDate}_{endDate}`. Cache hit/miss behaviour test. |
| NUnit tests — `PortfolioMetricsService.GetCumulativeStateTimeForPortfolio` + items variant | `Lighthouse.Backend/Lighthouse.Backend.Tests/Services/Implementation/PortfolioMetricsServiceTests.cs` | EXTEND | Same coverage as Team. Cross-scope parity test: identical fixture across team and portfolio scopes produces shape-identical responses. |
| NUnit tests — `TeamMetricsController` + `PortfolioMetricsController` | `Lighthouse.Backend/Lighthouse.Backend.Tests/API/TeamMetricsControllerTest.cs` + `PortfolioMetricsControllerTests.cs` | EXTEND | HTTP 400 on inverted date range; HTTP 400 on missing/empty `state` for items endpoint; HTTP 200 with empty body on team with no items; HTTP 200 with expected DTO shape on populated fixture; HTTP 200 with empty `items: []` on unknown state name; `[RbacGuard]` test asserts unauthenticated returns 401/403 matching existing `cycleTimePercentiles` tests. |
...
| E2E spec | `Lighthouse.EndToEndTests/tests/specs/flow/StateTimeCumulativeView.spec.ts` (new file) | NEW | 1 happy-path scenario: open team detail → Flow Metrics tab → see `CumulativeStateTimeChart` with bars → hover one bar and verify tooltip shows all fields → click the tallest bar → verify drill-down dialog opens with items table → Escape closes dialog → verify chart still visible. Per-state arithmetic correctness lives in NUnit (faster, deterministic) per the established sibling-1 pattern. |

### EXTEND / NEW summary

**EXTEND: 14** (existing files / classes / hooks / interfaces / metadata modules / test suites that gain additive code paths).
**NEW: 8** at the file/component level: `CumulativeStateTimeDto.cs`, `CumulativeStateTimeItemsDto.cs`, `CumulativeStateTimeChart.tsx`, `CumulativeStateTimeDrillDownDialog.tsx`, `CumulativeStateTime.ts` (TS models), `CumulativeStateTimeItems.ts` (TS models), the two new Vitest test files, the new E2E spec, and the new NUnit integration test for the sum invariant. (Counted at file granularity above; the records inside the DTO files are NEW types but counted as part of the single new file.)
**REUSE AS-IS: 13** (`IWorkItemStateTransitionRepository` sibling-1 port, `IWorkItemRepository`, `WorkItem.CurrentStateEnteredAt` + `State` + `StateCategory`, `BaseMetricsService.GetFromCacheIfExists`, `PercentileCalculator.CalculatePercentile`, `WorkItemBase.GetDateDifference`, MUI-X `<BarChart>` + `<ChartsContainer>` + `<ChartsTooltip>`, MUI `Dialog` + `DialogTitle` + `DialogContent`, `DataGridBase`, `WidgetShell`, `useRbac` hook, the existing `LogDateBoundaries` controller pattern, `GetEntityByIdAnExecuteAction` controller dispatch helper).

## Wave: DESIGN / [REF] Driving ports

| Method | Route | Auth | Status | Implementation pointer |
|---|---|---|---|---|
| GET | `/api/teams/{teamId:int}/metrics/cumulativeStateTime?startDate&endDate` | `[RbacGuard(TeamRead)]` (existing class-level) | NEW | `TeamMetricsController.GetCumulativeStateTimeForTeam` — mirrors `cycleTimePercentiles` shape and validation (line 119) |
| GET | `/api/teams/{teamId:int}/metrics/cumulativeStateTime/items?state={stateName}&startDate&endDate` | `[RbacGuard(TeamRead)]` | NEW | `TeamMetricsController.GetCumulativeStateTimeItemsForTeam` — additional `state` non-empty validation |
| GET | `/api/portfolios/{portfolioId:int}/metrics/cumulativeStateTime?startDate&endDate` | `[RbacGuard(PortfolioRead)]` | NEW | `PortfolioMetricsController.GetCumulativeStateTimeForPortfolio` — mirror of Team endpoint |
| GET | `/api/portfolios/{portfolioId:int}/metrics/cumulativeStateTime/items?state={stateName}&startDate&endDate` | `[RbacGuard(PortfolioRead)]` | NEW | `PortfolioMetricsController.GetCumulativeStateTimeItemsForPortfolio` — mirror of Team endpoint |
...
| Cache: existing `BaseMetricsService.GetFromCacheIfExists` | Existing in-process cache (post-`bug-5016-cache-thread-safety` thread-safe) | REUSE AS-IS — new cache key namespaces `CumulativeStateTime_…` and `CumulativeStateTime_Items_…` slot into the same scope-keyed cache |
...
| DDD-6 | US-03 counts: completed + ongoing + distinct item-count (non-additive case documented in tooltip wording) | ADR-022 |
...
| DDD-17 | Auth via existing class-level `RbacGuard` | DESIGN mechanical, no ADR |
...
**Totals: 27 rows. N EXTEND = 14, M REUSE-AS-IS = 13, K CREATE-NEW = 0 at the OVERLAP level.** The NEW entries in the Component decomposition table (8 new files: 2 DTOs, 2 components, 2 TS-model files, 2 test files, 1 E2E spec, 1 NUnit integration test file — counting at file granularity) all have ZERO existing overlap per the codebase greps below:

CODEBASE GREP FOR OVERLAP CANDIDATES THAT MIGHT JUSTIFY CREATE-NEW BUT WERE REJECTED:

- Searched for `CumulativeStateTime|cumulativeStateTime|StateCumulative|stateCumulative|StateTimeBar|stateTimeBar` across `Lighthouse.Backend` + `Lighthouse.Frontend/src` — zero production matches (confirmed via pre-DISCUSS code reality check; reconfirmed at DESIGN). No existing DTO, service, hook, or component duplicates the per-state cumulative concern.
...
- Searched for `*Drawer*.tsx` and `*SidePanel*.tsx` under `Lighthouse.Frontend/src/` — zero matches. Confirms ADR-023's "Dialog is the only in-codebase precedent" finding.
...
**Primary architectural commitment**: introduce four new HTTP endpoints (two per scope: bar + drill-down items), two new `protected` helpers inside the existing `BaseMetricsService` (parallel to sibling F's pattern), two new frontend components (a NEW chart widget using MUI-X `<BarChart>` and a NEW drill-down dialog using MUI `Dialog`), one new RAG function, three new metadata-module entries (`categoryMetadata`, `widgetInfoMetadata`, `ragRules`), and four new TypeScript model interfaces. Every other touchpoint is an additive extension of an existing class / hook / component / DTO / cache key namespace. NO new top-level routes. NO new external integration. NO new external library. NO new persistence (no new EF migration). NO premium gate. NO breaking change.

**Architecture pattern**: ports-and-adapters / hexagonal (unchanged). Reads transitions via sibling 1's repository port; reads work items via the existing repository port; no new port introduced. The computation helpers live as `protected` methods inside `BaseMetricsService` — intra-inheritance, NOT exposed via interface, parallel to sibling F's `ComputeAgeInStatePercentiles` (ADR-024).

**Walking skeleton path** (validates the architecture in one slice end-to-end): existing sibling-1 `WorkItemStateTransition` rows + existing `WorkItem.CurrentStateEnteredAt` → new `BaseMetricsService.ComputeCumulativeStateTime` helper → new `TeamMetricsService.GetCumulativeStateTimeForTeam` method (with D12 candidate query + cache wrap) → new `TeamMetricsController.GetCumulativeStateTimeForTeam` endpoint → new `MetricsService.getCumulativeStateTimeForTeam` TS method → extended `useMetricsData` ctx → new `CumulativeStateTimeChart` rendering on the team detail page. Slice 01 exercises every architectural seam end-to-end. Slice 02 adds portfolio parity + US-03 tooltip enrichment. Slice 03 adds US-04 per-item drill-down endpoints + dialog.
