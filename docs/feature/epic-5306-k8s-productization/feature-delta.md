...
Package + publish the chart to the public Helm repo (ADR-083). Added the **publish guard** (`chart/scripts/version-guard.sh`) — asserts `appVersion == values-enterprise.yaml image.tag`, that `NOTES.txt` surfaces `.Chart.Version` (agrees by construction), that `README.md` pins the exact chart version + appVersion, and **refuses to overwrite** a chart version already in the published index (no silent overwrite). Added **`chart/scripts/publish.sh`** — runs the guard, `helm package` into `docs/charts/`, `helm repo index --merge --url https://docs.lighthouse.letpeople.work/charts`. Added **`chart/README.md`** (the no-source `helm repo add` + `helm search repo` + `helm install --version` snippet, carrying the version surfaces). Added **`chart/templates/NOTES.txt`** chart-version line. Added **`.github/workflows/ci_chart.yml`** — standalone split-by-concern workflow: `validate` (helm lint default+enterprise, `helm unittest -f 'tests/unit/*.yaml'`, standalone-gate render guard) + `install-smoke` (kind cluster, `helm install` the real image, assert rollout Ready + `/health/ready` + SPA title) on every chart change; `publish` (manual `workflow_dispatch` — the maintainer's cheap finalization step — runs the guard, packages, indexes, commits `docs/charts/` so the existing `pages.yml` serves it). **Published the chart**: `docs/charts/index.yaml` + `docs/charts/lighthouse-0.1.1.tgz` committed.

Files: `chart/scripts/version-guard.sh` (new), `chart/scripts/publish.sh` (new), `chart/README.md` (new), `chart/templates/NOTES.txt` (+chart-version line), `.github/workflows/ci_chart.yml` (new), `docs/charts/index.yaml` + `docs/charts/lighthouse-0.1.1.tgz` (new — the published artifact).

Scenarios green (slice-04, `publish-and-docs.feature` @US-01):
- `External user installs from the published Helm repo with no source` (@real-io) — served `docs/charts/` over HTTP, `helm repo add` → `helm repo update` → `helm search repo lighthouse` shows `CHART 0.1.1 / APP 26.6.21.1`; `helm install --version 0.1.1 -f values-enterprise.yaml` from the **published .tgz** (no source dir) renders the full stack (API + MCP Deployments, Postgres StatefulSet, 3 Services, Ingress, ConfigMap, Secret). **Live kind dogfood**: installed from the packaged `.tgz` (`ingress.enabled=false`, bundled Postgres) — `api 1/1` + `postgres 1/1` Ready, `/health/ready` → `Healthy`, `/` → `<title>Lighthouse</title>`. Cluster torn down.
- `Publish refuses to silently overwrite an existing version` (@error) — with `0.1.1` already in the index, `version-guard.sh` exits 1 naming the un-bumped version; does not overwrite.
- `Chart version is consistent across all surfaces` — guard asserts Chart.yaml `0.1.1` == README snippet == NOTES.txt (`.Chart.Version`, prints `(chart 0.1.1)`) == published index; and `appVersion 26.6.21.1` == `values-enterprise.yaml image.tag`.

Quality gates (slice-04): `helm lint` clean (default + enterprise, only icon INFO); `helm unittest` 27/27; standalone-gate render guard green (1 Deployment, provider `postgres`, never sqlite); guard both paths exercised (pass + no-overwrite fail); live kind install from the published artifact Ready + serving.

**CI iterations (chart now at 0.1.1):** the first `Helm Chart` CI run surfaced two issues the local dogfood masked — both fixed:
1. `validate` — `helm unittest` plugin: helm-unittest v1.1.x's `plugin.yaml` uses the helm-v4-only `platformHooks` field and would not load/register on the pinned helm. Pinned `azure/setup-helm` v3.16.3 + helm-unittest **v1.0.3** (last pre-`platformHooks` release); verified against a real helm v3.16.3 sandbox (27/27). See `docs/ci-learnings.md` 2026-06-21.
2. `install-smoke` — the API CrashLooped with `SocketException(13) Permission denied`: the image bakes `Kestrel__Endpoints__Http__Url=http://+:80` (+443), which a non-root pod cannot bind on a standard cluster (`ip_unprivileged_port_start=1024`). The maintainer's host allowed unprivileged low-port binds, so local kind hid it. Fixed by overriding to unprivileged ports in the ConfigMap (`http://+:8080`, `https://+:8443`), `containerPort: 8080`, + regression render tests; re-verified in kind (`Now listening on http://[::]:8080`, `/health/ready` Healthy, SPA served). **Chart bumped 0.1.0 → 0.1.1** (patch — non-root port bind); broken 0.1.0 artifact dropped, index regenerated. See `docs/ci-learnings.md` 2026-06-21.
