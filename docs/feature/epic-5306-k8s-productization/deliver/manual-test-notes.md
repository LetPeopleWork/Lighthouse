...
| 12 | MCP OAuth discovery broken through the public ingress (mcp-http 1.3.0) | FIX-NOW (clients) | ✅ Fixed in lighthouse-clients (`96da0a7`, Option (a)): mcp-http now derives the `resource_metadata` URL from `X-Forwarded-Proto`/`X-Forwarded-Host` and mounts it under the same base path the request arrived on, and serves the metadata at both root and `/mcp/.well-known/...`. **No chart change needed** — the ingress already routes the `/mcp` prefix (incl. the well-known sub-path) to the MCP service; nginx sets X-Forwarded-Proto by default. Regression tests added (`bin.oauth.test.ts`); changeset = patch. Direct-to-service (http, root) preserved. **Follow-up:** pin `mcp.image` to the release that ships `96da0a7` once published. ADO #5362. Original root cause: `bin.ts:227` hardcoded `http://${req.headers.host}/.well-known/...`. |
