...
- **The per-record-class prerequisite for time-in-state.** Lighthouse reads state history from `metric_instance`, which carries rows only where a `Field value duration` metric definition sits on that class's *state* field. Definitions attach to concrete classes and never to `task` (ADR-123 D9), so every kind of work a team names needs its own. Measured on dev191338, 2026-08-01: stock ships one on `incident.incident_state` and one on `problem.problem_state`, and **none on `change_request.state`** — change_request ships definitions on `approval` and `type`, which are not states, so its spans are all dropped as unmapped labels and the class silently shows no time in state. Document: how to check (`Metric Definition` list, filter `Type = Field value duration`), how to add a missing one, and the two behaviours that surprise people — definitions record **forward only**, so records predating activation have no earlier spans; and a record's first span yields no transition (ADR-118), so a class needs a record to move **twice** after activation before any duration appears.
