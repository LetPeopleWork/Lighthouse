...
| Q4 | Should there be a specific right to create teams/portfolios? | Yes — canCreateTeam / canCreatePortfolio gate the Add buttons. Default false for non-System-Admins. System Admins always true. | Prevents accidental entity creation by scoped admins. **Superseded 2026-05-11 by `team-portfolio-creation-rights`** — non-System-Admins with at least one TeamAdmin/PortfolioAdmin role (direct or via group mapping) now have canCreateTeam/canCreatePortfolio true; the default-false rule applies only to Viewers. |
...
| WD-03 | 2026-05-10 | canCreateTeam/Portfolio = System Admin only (default) | Principle of least privilege; scoped admins manage existing entities, not create new ones. **Superseded 2026-05-11 by `team-portfolio-creation-rights`** — inferred-rights model in `RbacAdministrationService.CanCreateTeamAsync` / `CanCreatePortfolioAsync` is the service-level truth; this feature aligns the HTTP gate and frontend gating with it. |
