...
## Wave: DISTILL / [REF] Inherited commitments (slice 03 — staleness as a first-class Flow Signal)

Scope: **slice 03 only** (US-05 opt-in relocation, US-06 Stale Items widget, US-07 aging-chart staleness + retroactive D10 on the dialog badge). Slices 01 (US-01) and 02 (US-02/03/04) are SHIPPED to main and unchanged. This is a **frontend-heavy** slice: the only backend change is flipping the `Team`/`Portfolio` entity-initialiser defaults `7`/`14` → `0` plus the two integration-test expectations that pin those defaults. Every other touchpoint is React/TS.

| Origin | Commitment | DDD | Impact |
|--------|------------|-----|--------|
| DISCUSS#US-05 (D8 revised) | A newly-created team and a newly-created portfolio both default to `stalenessThresholdDays = 0` (staleness off until an admin opts in) | DDD-12 | Backend default-flip ATs assert `= 0`; existing slice-02 owners are NOT reset (PO 2026-05-25 — keep 7/14) so no data migration |
| DISCUSS#US-05 | The staleness opt-in lives in the "Flow Metrics Configuration" group (checkbox → on-enable seed default, on-disable → 0); the old "Flow Signals" group is removed from the team AND portfolio edit forms | DDD-12 | E2E walking-skeleton asserts the field appears in the Flow Metrics group seeded `5` (team), the old Flow Signals group is gone; FE unit (`FlowMetricsConfigurationComponent`) DEFERRED to DELIVER |
| DISCUSS#US-05 | Seed-on-enable default is owner-specific: `5` (team) / `14` (portfolio); `[0,365]` + RBAC gating unchanged | DDD-12 | E2E asserts team seed `5`; the `[0,365]` + `isTeamAdmin`/`isPortfolioAdmin` gating are slice-02 ATs (inherited, kept as regression guards) |
| DISCUSS#US-06 | A "Stale Items" overview widget (count + RAG + view-data) appears in the flow-overview category for team AND portfolio, modelled on Blocked Items | DDD-14 | E2E walking-skeleton asserts the widget count + RAG + view-data list; FE unit (`StaleOverviewWidget`, `computeStaleOverviewRag`) DEFERRED to DELIVER |
| DISCUSS#US-06 / D10 | The stale count EXCLUDES blocked items (blocked-and-over-threshold counts as Blocked, not Stale) | DDD-13 | E2E asserts a blocked-over-threshold item is not in the stale count; selector boundary unit (blocked-over → not stale) DEFERRED to DELIVER |
| DISCUSS#US-06 | The stale-widget "high" RAG reuses Blocked's `count >= 2` (PO 2026-05-25) | DDD-14 | One-line constant at DELIVER; widget RAG unit (`computeStaleOverviewRag`) DEFERRED to DELIVER |
| DISCUSS#US-07 | Stale items render red in the Work Item Aging Chart (same emphasis as blocked); clicking a bubble shows red Time-in-State; chart RAG factors `isStale` like `isBlocked` | DDD-15 | E2E walking-skeleton asserts a red stale bubble + red Time-in-State on click; aging-chart `isStale` + `computeWorkItemAgeChartRag` units DEFERRED to DELIVER |
| DISCUSS#US-07 / D10 | Blocked-excludes-stale is applied UNIFORMLY on every surface, INCLUDING retroactively to the existing WIP-dialog Time-in-State badge (blocked-over-threshold no longer red) | DDD-13 | E2E asserts a blocked-over-threshold item is never red-as-stale on chart, widget, or dialog badge; `TimeInStateBadge` `isBlocked`-prop + `deriveStaleness` selector units DEFERRED to DELIVER |
| ADR-026 / DDD-13 | A single client-side pure `deriveStaleness(item, thresholdDays, now) → boolean` is the sole home of the predicate + blocked-exclusion; the three surfaces can never disagree; DDD-8 (client-side, no sync) upheld; server-side aggregates OUT of scope | DDD-13 | Asserted at the FE selector unit level (DEFERRED to DELIVER) + cross-surface E2E; no backend round-trip on derivation |

## Wave: DISTILL / [REF] Scenario list with tags (slice 03)

| # | Scenario | Tier / Layer | Tags | AC covered | Status |
|---|---|---|---|---|---|
| 1 | `GetTeamSettings_NewlyCreatedTeam_ExposesDefaultStalenessThresholdOfZero` | Tier A — integration (layer 4) | `@driving_port` `@US-05` | Team default flips 7 → 0 (opt-in) | **AUTHORED** — NEW `[Ignore]` RED scaffold |
| 2 | `GetPortfolioSettings_NewlyCreatedPortfolio_ExposesDefaultStalenessThresholdOfZero` | Tier A — integration (layer 4) | `@driving_port` `@US-05` | Portfolio default flips 14 → 0 (opt-in) | **AUTHORED** — NEW `[Ignore]` RED scaffold |
| 3 | E2E — team admin opts staleness in from the "Flow Metrics Configuration" group; field seeded `5`; old "Flow Signals" group gone | Tier A — E2E (layer 6), demo-data | `@walking_skeleton` `@driving_port` `@real-io` `@US-05` | US-05 relocation + opt-in + seed `5` + group removal | **AUTHORED** — `test.fixme` demo-data scaffold |
| 4 | E2E — Stale Items widget counts a stale item (count + RAG + view-data); blocked-over-threshold counted by Blocked not Stale | Tier A — E2E (layer 6), demo-data | `@walking_skeleton` `@driving_port` `@real-io` `@US-06` | US-06 widget + D10 blocked-excludes-stale | **AUTHORED** — `test.fixme` demo-data scaffold |
| 5 | E2E — stale item red in aging chart + red Time-in-State on bubble click; blocked-over-threshold never red-as-stale on any surface | Tier A — E2E (layer 6), demo-data | `@walking_skeleton` `@driving_port` `@real-io` `@US-07` | US-07 chart red + bubble detail + D10 cross-surface | **AUTHORED** — `test.fixme` demo-data scaffold |
| 6 | `deriveStaleness` selector boundary: threshold 0 → not stale; at-threshold → not stale; one-over → stale; blocked-over → not stale | Unit (Vitest, layer 1) | `@property` `@US-06` `@US-07` `@D10` | ADR-026/DDD-13 selector + blocked-exclusion correctness | **DEFERRED to DELIVER** (references not-yet-existing `deriveStaleness` export — would red `pnpm build`) |
| 7 | `StaleOverviewWidget` renders count card + RAG + view-data (clone of `BlockedOverviewWidget`) | Unit (Vitest/RTL, layer 1) | `@US-06` | DDD-14 widget render | **DEFERRED to DELIVER** (references not-yet-existing `StaleOverviewWidget`) |
| 8 | `computeStaleOverviewRag(count, hasStaleConfig, terms)`: red if `!hasStaleConfig`, red `≥2`, amber 1, green 0-with-config | Unit (Vitest, layer 1) | `@US-06` | DDD-14 RAG semantics (reuses Blocked `≥2` high) | **DEFERRED to DELIVER** (references not-yet-existing `computeStaleOverviewRag`) |
| 9 | `TimeInStateBadge` routes its red decision through `deriveStaleness` + new `isBlocked` prop; blocked-over-threshold no longer red | Unit (Vitest/RTL, layer 1) | `@US-07` `@D10` | DDD-15 retroactive D10 on the dialog badge | **DEFERRED to DELIVER** (references not-yet-existing `isBlocked` prop / selector wiring) |
| 10 | `WorkItemAgingChart` reds stale bubbles (`isStale` from selector); blocked precedence; Time-in-State column on the chart dialog | Unit (Vitest/RTL, layer 1) | `@US-07` `@D10` | DDD-15 chart shape `{ workItemAge, isBlocked, isStale }` | **DEFERRED to DELIVER** (references not-yet-existing `isStale` chart-item field) |
| 11 | `computeWorkItemAgeChartRag` factors `isStale` symmetrically with `isBlocked` | Unit (Vitest, layer 1) | `@US-07` | DDD-15 chart RAG extension | **DEFERRED to DELIVER** (references not-yet-existing extended item shape) |
| 12 | `FlowMetricsConfigurationComponent` staleness opt-in: unchecked default, on-enable seed-default field, on-disable → 0 | Unit (Vitest/RTL, layer 1) | `@US-05` | DDD-12 opt-in pattern | **DEFERRED to DELIVER** (references not-yet-existing `stalenessThresholdDays` binding + `stalenessSeedDefault` prop) |
| 13 | `ForecastSettingsComponent` / `EditPortfolio` lose the old "Flow Signals" staleness block | Unit (Vitest/RTL, layer 1) | `@US-05` | DDD-12 group removal | **DEFERRED to DELIVER** (asserts removed markup; co-located with the relocation change) |

Error/edge ratio: counting the AUTHORED-here scenarios (#1–5), the boundary/precedence coverage (#1 and #2 assert the opt-in/off default; #4 and #5 each carry the D10 blocked-excludes-stale negative case) gives 4 of 5 carrying an edge/negative dimension = **80%**, well over the 40% target. The DEFERRED FE-unit suite (#6) explicitly enumerates the four boundary cases (threshold 0 / at-threshold / one-over / blocked-over). Tier B (state-machine PBT) is correctly SKIPPED: per Mandate 10 "skip when" — US-05 is config-shaped (single opt-in integer), and US-06/US-07 are per-render projections of a pure selector with no chained ≥3-scenario state machine to model. Per the C#/TS polyglot rows, no PBT/`@given`/`RuleBasedStateMachine`/state-delta machinery applies (those are Python-pilot-specific, Mandates 8–10); layer-4 backend ATs and layer-6 E2E are example-only (Mandate 11).

## Wave: DISTILL / [REF] WS strategy (slice 03)

Inherits "Type A (additive walking skeleton)". Under the project Architecture of Reference (now recorded in `docs/architecture/atdd-infrastructure-policy.md`, bootstrapped this run): **Driving port** = real HTTP via `WebApplicationFactory<Program>` (backend default-flip ATs) and the production React app via Playwright (E2E); **Driven internal** (EF `LighthouseAppContext`) = real adapter through the test factory; **Driven external** (connector APIs) = **seeded DEMO data**, NOT live connector syncs.

**Demo-data E2E (standing project rule).** The three walking-skeleton scaffolds (#3–5) use `loadDemoScenario(request, DEMO_SCENARIO_ID=0)` + `waitForBackgroundUpdates(request)` — the deterministic "When Will This Be Done?" scenario seeding **Team Zenith** (the same mechanism `tests/specs/teams/ForecastFilter.spec.ts` uses). This converges away from the held slice-02 walking-skeletons, which were `testWithUpdatedTeams` (live ADO/Jira syncs) and flaked on 0-WIP + the overview Search box being hidden during active background syncs (their own `test.fixme` notes record this). The `tests/helpers/api/{teams,portfolios}.ts` create-helpers already send `stalenessThresholdDays` (verified: `teams.ts:32` sends `0`). The scaffolds are `test.fixme` and will be RUN live locally in DELIVER (never committed unrun, per project rule) and only un-fixme'd once green.

**Demo-seed gap flagged for DELIVER** (NOT a DISTILL blocker): the US-06/US-07 scaffolds (#4, #5) assert an over-threshold item AND a blocked-over-threshold item exist in the seeded set. Demo scenario 0 (Team Zenith) seeds in-progress items, and the scaffolds set the threshold to `1` so realistically-aged items qualify; whether scenario 0 reliably includes a **blocked** in-progress item over threshold is a DELIVER verification (confirm the demo seed's state ages + blocked tag, or pick/extend a demo scenario that guarantees one). This is the first thing DELIVER confirms when un-fixme-ing #4/#5.

## Wave: DISTILL / [REF] Adapter coverage (slice 03)

This slice introduces **no new driven adapter**. It reuses the slice-01/02 persistence + capture pipeline unchanged (transition capture, `CurrentStateEnteredAt`, `StalenessThresholdDays` column, the read DTO). The new logic is a **pure client-side selector** (`deriveStaleness`) with no I/O — Mandate 6 (every driven adapter has a real-I/O scenario) is satisfied vacuously for new adapters and inherited for existing ones.

| Driven adapter / surface | Covered by | Real-IO scenario? |
|---|---|---|
| `Team`/`Portfolio` `StalenessThresholdDays` default + persistence (EF) | Backend default-flip ATs (#1, #2) read the persisted default through the real EF context via the settings endpoint; the slice-02 PUT round-trip ATs (inherited) cover write | YES — real EF round-trip via `WebApplicationFactory` |
| `deriveStaleness` selector (pure, no I/O) | FE unit #6 (DEFERRED to DELIVER) + cross-surface E2E #3–5 | N/A — pure function, no adapter |
| Stale Items widget / aging-chart staleness / dialog badge (FE render of the selector) | E2E walking-skeletons #3–5 (demo-data, real React app) + FE units #7–13 (DEFERRED to DELIVER) | YES (E2E) — real production React app over seeded demo data |

No external integration; this slice reads only Lighthouse-internal persisted data. **No contract test required** (confirmed at DESIGN slice-03 wave-decisions summary).

## Wave: DISTILL / [REF] Test placement + precedent justification (slice 03)

| Artifact | Path | Precedent |
|---|---|---|
| Team default-flip RED scaffold (1 test) | `Lighthouse.Backend/Lighthouse.Backend.Tests/API/Integration/TeamStalenessThresholdSettingsIntegrationTest.cs` (EXTENDED — NEW `…ExposesDefaultStalenessThresholdOfZero`, `[Ignore]`) | Reuses the same fixture: `SeedTeam()`, `TryGetInt`, `client.AsTeamAdmin`. Sits beside the live slice-02 `…OfSevenDays` test which it will REPLACE at GREEN. |
| Portfolio default-flip RED scaffold (1 test) | `Lighthouse.Backend/Lighthouse.Backend.Tests/API/Integration/PortfolioStalenessThresholdSettingsIntegrationTest.cs` (EXTENDED — NEW `…ExposesDefaultStalenessThresholdOfZero`, `[Ignore]`) | Same; reuses `SeedPortfolio()`, `client.AsPortfolioAdmin`. Sits beside the live `…OfFourteenDays` test. |
| E2E walking-skeleton scaffolds (3) | `Lighthouse.EndToEndTests/tests/specs/flow/TimeInStateAndStaleness.spec.ts` (EXTENDED — 3 NEW `test.fixme`, demo-data) | Mirrors `tests/specs/teams/ForecastFilter.spec.ts` (`loadDemoScenario` → `waitForBackgroundUpdates` → `goToTeam("Team Zenith")` → `goToMetrics` → `switchCategory` → `getWidgetByName` → `openDialog`). The held slice-02 `testWithUpdatedTeams` scenarios are left UNTOUCHED. |
| E2E POM extensions | `tests/models/teams/TeamEditPage.ts`, `tests/models/portfolios/PortfolioEditPage.ts` (`stalenessEnableCheckbox`, `enableStaleness`, `legacyFlowSignalsGroupHeader`); `tests/models/metrics/MetricsPage.ts` (`StaleItemsOverview` widget name + `["Stale Items Overview","staleOverview"]` registration in `flow-overview`; `MetricsWidget` gains `staleOverviewCount`/`getStaleOverviewCount`/`ragStatusIndicator`/`getRagStatus`/`staleAgingBubbles`/`countStaleAgingBubbles`) | POM-only rule. The threshold-field locator (`getByLabel("Staleness Threshold (days)")`) is REUSED — DESIGN keeps the field label, so the locator survives the relocation; what changes is that it appears inside the Flow Metrics group after the opt-in checkbox, hence the new `enableStaleness()` helper. Stale widget reuses the existing `MetricsWidget.openDialog`/`ViewDataButton` seam. |

**Black-box / compile-safety note**: the two backend RED scaffolds reference NO not-yet-existing C# symbol — `stalenessThresholdDays` is read from the response JSON via the existing `TryGetInt`; the seed creates a `Team`/`Portfolio` with no explicit threshold so it picks up the entity-initialiser default (7/14 today, 0 after the US-05 flip). The E2E POM additions reference only Playwright locator strings (which always compile). **Verified this pass**: `dotnet build Lighthouse.Backend.Tests -warnaserror` → 0 warnings / 0 errors; the two new tests report **Skipped** and the 12 live slice-02 threshold tests still **Pass** (default still 7/14 — the flip is a US-05 DELIVER step); `npx tsc --noEmit` on the E2E project → exit 0. Suite stays green.

## Wave: DISTILL / [REF] AT files created / extended (slice 03)

**Created / extended this pass (compile-safe, suite green):**
- `Lighthouse.Backend.Tests/API/Integration/TeamStalenessThresholdSettingsIntegrationTest.cs` — EXTENDED: NEW `GetTeamSettings_NewlyCreatedTeam_ExposesDefaultStalenessThresholdOfZero`, `[Ignore("pending DELIVER: US-05 default flip …")]`.
- `Lighthouse.Backend.Tests/API/Integration/PortfolioStalenessThresholdSettingsIntegrationTest.cs` — EXTENDED: NEW `GetPortfolioSettings_NewlyCreatedPortfolio_ExposesDefaultStalenessThresholdOfZero`, `[Ignore("pending DELIVER: US-05 default flip …")]`.
- `Lighthouse.EndToEndTests/tests/specs/flow/TimeInStateAndStaleness.spec.ts` — EXTENDED: 3 NEW `test.fixme` demo-data walking-skeletons (US-05/06/07); held slice-02 scenarios untouched.
- `Lighthouse.EndToEndTests/tests/models/teams/TeamEditPage.ts` + `tests/models/portfolios/PortfolioEditPage.ts` — EXTENDED (opt-in checkbox + `enableStaleness` + legacy-group header readers).
- `Lighthouse.EndToEndTests/tests/models/metrics/MetricsPage.ts` — EXTENDED (Stale widget name + registration; widget count / RAG / stale-bubble readers).
- `docs/architecture/atdd-infrastructure-policy.md` — bootstrapped this run (was absent); records the C#/TS port→mechanism mapping including demo-data E2E.

**DEFERRED to DELIVER (would red `pnpm build` now — reference not-yet-existing TS exports/props, per the slice-02 "DEFERRED to DELIVER (would red the build now)" precedent):**
- **US-05** — `FlowMetricsConfigurationComponent` staleness opt-in unit test (binds a not-yet-hoisted `stalenessThresholdDays` on `IBaseSettings` + a not-yet-added `stalenessSeedDefault` prop); `ForecastSettingsComponent.test.tsx` / `EditPortfolio.test.tsx` "Flow Signals removed" assertions (co-located with the relocation change). Also the backend default-flip itself (entity-initialiser `7`/`14` → `0` + flipping the live slice-02 `…OfSevenDays`/`…OfFourteenDays` expectations) is a US-05 DELIVER RED step — see the gate below.
- **US-06** — `deriveStaleness` selector boundary unit (`deriveStaleness.test.ts` — references the not-yet-existing `deriveStaleness` export); `StaleOverviewWidget` render unit (`StaleOverviewWidget.test.tsx`); `computeStaleOverviewRag` unit (`ragRules.test.ts` — references the not-yet-existing function).
- **US-07** — `TimeInStateBadge` blocked-excludes-stale unit (references the not-yet-existing `isBlocked` prop + selector wiring); `WorkItemAgingChart` stale-red + Time-in-State-on-click unit (references the not-yet-existing `isStale` chart-item field); `computeWorkItemAgeChartRag` `isStale` extension unit.

No production code, no FE unit tests, no roadmap authored this pass — DELIVER is the sole author of production code and the deferred FE units (RED→GREEN), and `/nw-roadmap` follows separately.

## Wave: DISTILL / [REF] Pre-DELIVER fail-for-the-right-reason gate (slice 03)

Un-ignore / un-fixme each AT one at a time; confirm the failure is MISSING_FUNCTIONALITY, not a setup/compile/fixture error (BLOCK and fix the test otherwise):

1. **Backend default-flip (#1, #2) — a paired RED→GREEN step.** Un-ignore the NEW `…ExposesDefaultStalenessThresholdOfZero` test. It will FAIL today for the RIGHT reason: a newly-created owner still gets the entity-initialiser default 7/14, so the `Is.Zero` assertion fires (`threshold == 7`/`14`, not 0). To GREEN it: flip `Team.cs:23` (`= 7;` → `= 0;`) and `Portfolio.cs:31` (`= 14;` → `= 0;`) per DDD-12. In the SAME change, flip the const + rename the now-stale live test — `DefaultTeamThresholdDays = 7` → `= 0` and replace/retire `…ExposesDefaultStalenessThresholdOfSevenDays`; likewise `DefaultPortfolioThresholdDays = 14` → `= 0`, `…OfFourteenDays`. WRONG RED: `JsonDocument.Parse` throws, `stalenessThresholdDays` absent (settings payload regression), 403/500. **Existing-row reset is explicitly NOT done** (PO 2026-05-25): only the go-forward entity default changes; teams/portfolios created in slice 02 keep their 7/14.
2. **Fixture Theater check on the flip.** When #1/#2 flip to GREEN, `git diff --stat` MUST show `Team.cs` + `Portfolio.cs` changed. A green that touches only the test file means the const was edited without flipping the initialiser — BLOCK.
3. **E2E (#3–5) un-fixme LAST**, start a local app (premium-license dev seed per memory `reference_premium_license_dev_seed` if any portfolio path hits `[LicenseGuard]`), `loadDemoScenario(0)` + `waitForBackgroundUpdates`, run live, and confirm RED for the right reason — Flow Metrics group has no staleness opt-in / the Stale widget is absent / the aging chart has no red stale bubble — then drive each green. Per project rule + memory, NEVER commit a Playwright spec not RUN locally against a started app. **Confirm the demo seed** (scenario 0, Team Zenith) actually contains an over-threshold item and a blocked-over-threshold item for #4/#5; if not, pick/extend a demo scenario that guarantees one (see the demo-seed gap flagged in WS strategy).
4. **The DEFERRED FE units are authored RED→GREEN in DELIVER**, not here (authoring them now reds `pnpm build`). DELIVER writes each `*.test.tsx`/`*.test.ts` RED against the new export/prop, then implements to GREEN, one cycle at a time. The `deriveStaleness` selector unit is authored FIRST (it is the single home of the predicate + blocked-exclusion that the three surfaces consume — ADR-026/DDD-13).
5. **Skip-to-push discipline (memory `feedback_green_before_push_skip_acceptance`)**: each story is CI-verifiable green before push; keep the not-yet-passing E2E `test.fixme` and any not-yet-passing FE unit skipped to push green, then un-skip to resume Outside-In TDD. Manually verify each story in DELIVER.

## Wave: DISTILL / [REF] Reconciliation + infrastructure policy notes (slice 03)

- **Wave-decision reconciliation: PASSED — 0 contradictions.** DISCUSS slice-03 (revised D8 = 0/0 opt-in; D10 blocked-excludes-stale) and DESIGN slice-03 (DDD-12 default flip + relocation; DDD-13 single selector; DDD-14 widget; DDD-15 aging chart) agree across US-05/06/07. The DDD-5 / D8 supersession is an explicitly-flagged amendment (default-value clause only; placement on `WorkTrackingSystemOptionsOwner` intact), not a live contradiction. DDD-8 (client-side, no-sync) is upheld and extended by ADR-026, not contradicted. No `docs/feature/time-in-state-and-staleness/devops/` delta exists — default environment assumptions apply (warn, not block).
- **Demo-data E2E convergence (follow-up 1, DELIVER task — NOT done here):** the held slice-02 walking-skeletons (`testWithUpdatedTeams`, live-data, currently `test.fixme`, held uncommitted) and the slice-03 walking-skeletons (demo-data) both target the same spec file. Their reconciliation onto the deterministic demo-data approach is its own DELIVER task — this DISTILL pass authored ONLY the slice-03 demo-data scaffolds and did NOT rewrite or recommit the held slice-02 specs.
- **Infrastructure policy:** bootstrapped this run at `docs/architecture/atdd-infrastructure-policy.md` (was absent). C#/.NET + React/Playwright, NOT the Python pilot — the Python-pilot artifacts (`tests/common/state_delta.<ext>`, `assert_state_delta` Universe assertions, Hypothesis/PBT harnesses, `__SCAFFOLD__` stubs) do not apply; the C#/TS polyglot rows govern (NUnit `[Ignore]` + Playwright `test.fixme` skip markers; backend ATs black-box example-based via `WebApplicationFactory<Program>`; no PBT/state-delta at any layer; layer-4/layer-6 sad paths example-based per Mandate 11). Driving = `WebApplicationFactory<Program>` (backend) / production React app (E2E); Driven internal = EF context via the test factory; Driven external = seeded demo data (E2E) / `Mock<ILicenseService>` + `Mock<IWorkTrackingConnector>` (backend).

## Wave: DELIVER / [REF] Outcomes (slice 03 + #5088 — feature complete)

Slice 03 (US-05/06/07) and the #5088 fast-follow shipped to `main`. All 7 E2E walking-skeletons are live-verified on demo data — **zero `test.fixme` remaining** in `TimeInStateAndStaleness.spec.ts`.

| Story / item | Outcome | Commit(s) |
|---|---|---|
| US-05 (#5089) | Staleness is an opt-in Flow Metric: default `0` (off) for new team + portfolio; relocated into the generic `FlowMetricsConfigurationComponent` (checkbox → seed 5 team / 14 portfolio → 0 on disable); standalone "Flow Signals" group removed. Existing slice-02 owners keep 7/14 (no reset). | `b367cbc7` |
| US-07 (#5091) | Single client-side `deriveStaleness` selector (DDD-13 / ADR-026) is the sole home of the staleness predicate + blocked-excludes-stale; consumed by the dialog badge (retrofit), the Stale Items widget, and the aging chart (red stale bubbles + Time-in-State on bubble click + `isStale` in chart RAG). | `831245b6`, `eb2aae44` |
| US-06 (#5090) | "Stale Items" overview widget cloned from Blocked Items (count + RAG + view-data); count excludes blocked items. | `e0125152`, `2d760c15` |
| Demo-seeder | Optional CSV "Current State Since" column (`StateEnteredDateHeader`) → connector emits `SyncedTransitions` → `RefreshWorkItems` derives `CurrentStateEnteredAt` (DDD-6 intact); demo CSVs carry `StateEnteredDate` so the whole feature is demoable. | `1200f791` |
| #5088 portfolio Feature transitions | Portfolio Time-in-State column shows real data. Jira + ADO already captured Feature transitions via the shared converter (`expand=changelog` / throttled revisions); **Linear** gap fixed — `Project.history` added to the projects query, mapped to states (live-verified). | `aa325466`, `daa5f4a0` |
| Fixes (surfaced by live E2E) | Overview edit-POM redirect-race de-flake; React #185 render loop in the aging chart (unstable `now` default → `useMemo`). | `e3403e26`, `eb2aae44` |

**DDD refinement**: DDD-3 — `IWorkTrackingConnector.SupportsTransitionHistory` became a connection-parameterized method (`bool SupportsTransitionHistory(connection)`) so CSV answers per-connection (true only when the state-since column is configured); all four connectors updated. Placement (DDD-5), single-writer (DDD-6), client-side comparison (DDD-8) all upheld.

**Verification**: 7/7 E2E walking-skeletons green on demo data (Team Zenith TZ-040 blocked+stale / TZ-039 stale / TZ-038 fresh; Project Apollo AP-004 stale); backend suite green; SonarCloud gate green; DDD-6 ArchUnit (RefreshWorkItems sole writer) holds. US-05 manually verified by the PO.

**ADO**: #5026/27/28 (slice 01/02) Closed; #5089/90/91 (slice 03) Resolved; #5088 Resolved. Sibling Epic-4144 features (`aging-pace-percentiles`, `state-time-cumulative-view`) are separate.
