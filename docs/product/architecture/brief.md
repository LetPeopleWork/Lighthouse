...
**Two typed read endpoints on `MetricsController` (ADR-108)**: `GET .../metrics/percentiles-over-time?horizon={30|60|90}` → `{recordedAt, metricType, p50..p95}[]` and `GET .../metrics/process-behavior-over-time?type={Throughput|WorkItemAge|Wip|CycleTime|Arrivals|FeatureSize}` → `{recordedAt, unpl, average, lnpl}[]`, both team + portfolio scope (D8; Feature-Size portfolio-only). Each widget gets an honestly-typed DTO matching its toggle (`[WIA|CT-30|CT-60|CT-90]` vs the PBC type toggle), mirroring the two-table + two-widget split; a polymorphic envelope (FE branches on `seriesType`) and a single all-series call (over-fetch, coupled lifecycles) were rejected. Read-only driving ports; recording writes flow through the ADR-107 handlers' driven ports. Empty series on a zero-snapshot owner → the D6 honest empty-state, never a broken axis. Contract change is **additive** (2 new GET actions + 2 DTOs) ⇒ no CLI/MCP client version gate; reads inherit the existing `MetricsController` read gate ⇒ no RBAC change.
