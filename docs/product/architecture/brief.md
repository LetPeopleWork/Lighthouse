...
- **Per-state pace bands use a cumulative reached-at-least-this-state population (connector-agnostic)** — for each mapped `Doing` state `S` (in the team's configured `DoingStates` order), the band's population is every completed-in-window item that reached `S` or any state at-or-after `S`, and each item contributes its last-exit cumulative age (`(last exit from S).TransitionedAt − StartedDate`); items that skipped `S` or were captured only downstream are imputed from their earliest at-or-after exit. Because each state's population is a superset of the next state's on one monotonic age clock, the bands rise monotonically left→right and the rightmost band ≈ the cycle-time percentile line **by construction, on every connector**. The computation operates only on the **normalized `WorkItem.SyncedTransitions` model** plus the team's mapped `DoingStates` — there is **no per-connector branching** (no `switch (WorkTrackingSystem)`); the non-linear flows that this corrects (skips, backward moves, unmapped/intermediate statuses, faster-downstream cohorts) are a data shape any source can produce, not a Jira special case. This supersedes the original per-state-exit-population metric, whose monotonicity / last-column promises were false on non-linear data (bug #5145). The mapped-state filter is scoped to the pace path only (`BuildWorkflowStateOrder` is pace-exclusive); the sibling cumulative-state-time chart is unaffected. No schema / contract / cache-key change. See ADR-047 (supersedes the ADR-019 §1/§3 metric).
...
- [ADR-047](./adr-047-aging-pace-cumulative-population.md): Aging-Pace Bands — Cumulative Reached-At-Least-This-State Population, Connector-Agnostic (bug #5145; supersedes the ADR-019 §1/§3 per-state-exit metric)
