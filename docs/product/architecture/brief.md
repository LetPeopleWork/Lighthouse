...
Enum-ordinal note (all slices): `MetricType` — and, when it lands, `ProcessBehaviorMetricType` — is persisted as its **integer ordinal**. New members are **appended only**; reordering or renumbering silently re-maps every already-shipped snapshot row to a different metric family, with no compiler error, no failing test and no migration to review.
