...
C4Context
  ...
  System_Ext(posthog, "PostHog Cloud EU", "Operated by PostHog Inc. Data rests in the EU")
...
## Application Architecture — story-6053-reconstruct-over-time-history (User Story 6053)

Feature: story-6053-reconstruct-over-time-history — a second write path that **recomputes** a missing over-time day from stored work items, so the two Predictability widgets show the trend an instance's data already supports instead of the handful of days the recorder happened to catch. DESIGN 2026-09-22, interaction mode PROPOSE. **No new bounded context, no new table, no new route, no new DTO, no EF migration, no RBAC change, no external integration, no new technology.** Default style unchanged (modular monolith + ports-and-adapters, OOP). Four slices; slice 01 is the thinnest end-to-end proof (CT-30, team scope), not a skeleton — every layer already exists and is under test.

**The problem is measured, not reported.** The dev database holds 36 percentile rows across **4 distinct days** and 42 PBC rows across 4, spanning 17 calendar days — every one a weekend or a Monday, because the forward-only recorder (ADR-107) only writes when the instance is up. Against that sit 615 closed work items spanning a full year. Four recorded days against roughly 365 reconstructible ones; one portfolio has exactly three, which is the community report ("only 3 entries") reproduced literally. On a standalone install this is the normal case.

**Reconstruction is recomputation, not synthesis, and the distinction carries the whole design (D1).** A missing day `D` is the recorder's own service call with the window shifted to `(D − horizon, D)`, stamped `RecordedAt = D`. SPIKE-01 reproduced all four genuinely recorded days exactly, so the mechanism holds — while recording that the probe discriminates weakly (a near-degenerate distribution, three of four days sharing a tuple, and **no configuration change in the covered period**). ADR-109's rejection of real-tenant backfill is therefore **narrowed to synthesis** rather than overturned: `DemoPercentilesBackfillHandler` invents values from a deterministic wave and stays demo-only, untouched.

**A read may ask; only the filler may write (ADR-207).** The trigger is the series read detecting a gap in the window it was asked for (D2), which overturns ADR-108's slice-03b "never a recompute trigger" property — amended there, narrowed to "read-only **in the response**". The seam is a **decorator over each series query port**, not the controller (four call sites and a controller that computes) and not the query implementation (a component whose name would stop describing it). The decorator delegates the read, hands the rows and the window to `IOverTimeGapReconciler` — one void method, no repository — and returns the rows untouched. **Nothing reachable from a controller action depends on a snapshot repository**, so "a GET wrote to the database on the request thread" is structurally unavailable rather than merely untested. Gap detection is a scan over rows already materialised plus a dictionary lookup; a read that finds nothing missing runs **no additional query**, and a read that has found days to ask for runs **exactly one** since 2026-09-24 — the opt-in switch below, by primary key. The data floor and the owner's last-observed ceiling are both applied inside the background pass (the read loads no owner), because the KPI budget is 50 ms over today's p95.

**The carrier is a filler of this feature's own, not `IUpdateQueueService` — the one place the reuse default was overridden, and it is evidence-backed.** `UpdateQueueService` holds a single `Channel<Func<Task>>` drained by a single reader: ADR-195's three-lane fix for exactly that starvation was shipped and **reverted** with story-5877 (see the note above), so ADR-195/196/197 describe a design that is not in the code. A 90-day multi-family pass — 5–6 s on 621 items, unbounded above that — would therefore sit in front of every Team and Portfolio refresh, ranking a cosmetic chart backfill ahead of the product's core function. Two further costs settle it: a new `UpdateType` is a **user-visible cancellable Task-Manager row** needing a verb, a noun and a terminology key in three `satisfies Record<UpdateTaskType, …>` tables plus a hand-maintained TypeScript union with no compile-time link to the backend enum; and an admitted key keeps `HasActiveWork()` true, which is what holds database maintenance off. What is reused is the *pattern* — bounded channel, single reader, per-key in-flight set, scope per pass, drain on shutdown.

**The unit of work is `(owner, window)`, never a family or a day.** One ask per owner per read; the pass covers every percentile family and every PBC family for the scope. That is what makes "switching tabs does not re-trigger reconstruction" a structural property rather than a behaviour to test for, and it is what makes every tab span the same period — the thing the combined widget was built for. **D4's cap is locked at 90 days *per pass*, not 90 days of reach**, plus a wall-clock budget. As built (corrected 2026-09-24; ADR-207 §5 had described a newest-first walk that was never written): a pass walks the missing days **oldest first**, steps over days outside the floor and ceiling without counting them, and counts the **days it works out**, not the days it writes — counting at the read handed a period reaching past the owner's history nothing but unfillable days, and newest-first would open every visit to an unsynced owner with days it refuses again. A year-wide picker fills across successive loads, oldest stretch first, and the walk is **resumable by construction** because the thing that triggers it is a read that will happen again. That is the honest answer to SPIKE-01's explicit inability to extrapolate cost from 621 items on SQLite to tens of thousands. Cost converges to zero once a window is dense; a per-owner in-memory **reconciliation memo** (resolved floor + refused days, invalidated by the refresh events) is what stops a permanently-unfillable day re-enqueueing a pass forever, and it is optimisation-only — losing it to a restart costs one wasted pass. Idempotency is three layers: the in-flight set at enqueue, **fill-if-absent** at write (never update-in-place — a recorded day is left exactly as recorded), and the ADR-106 unique index as the collision backstop, absorbed per day. Two replicas can both run a pass; that is wasted CPU, never a duplicate row.

**A past day is computed by the recorder's own code (ADR-208).** The per-day computation and the family descriptors — `CycleTimeHorizons`, `WorkItemAgeHorizons`, `TeamReaders`/`PortfolioReaders`, `LookbackDaysFor` — move out of the two recording handlers into two shared writers, each offering two **named** operations (`RecordToday` overwrites; `FillDayIfAbsent` writes only if absent) rather than one operation with a mode flag. One code path makes D1 and D6 structural instead of tested-for, and one family list per scope turns "the sets match `TeamReaders`/`PortfolioReaders` exactly" into an assertion about one list — the drift ADR-109's slice-02 amendment already recorded happening to the demo backfill's guard. **Every today-anchor on the PBC path becomes an explicit as-of day defaulting to `Clock.Today`**, threaded to `BaselineValidationService.Validate` and the `DoneItemsCutoffDays` cutoff: validating day D against today answers a different question, and its failure is silent — a pinned-baseline owner can go `BaselineInvalid`, hit the `Status != Ready` gate, and produce nothing that looks exactly like "the data didn't support it". A pinned baseline's correct series is **flat limits, not an empty one**. **The absence gate applies to both paths**, answering the adjacent question DISCUSS left to DESIGN: gating only reconstruction would make a day's row depend on which path reached it first, and fill-if-absent would make the recorder's all-zero row permanent — a standing invisible exception to D6. That is a **behaviour change to shipped code** (the forward recorder stops writing all-zero percentile rows) and is named in the release notes and the metrics docs; existing rows stay, with no repair migration.

**Frontend**: copy only, in slice 04. The forward-only empty-state sentence becomes false in two new ways, and both are covered by one sentence true of both states ("some days in this range have no recorded or reconstructible value") rather than by a `hasHistory` field — which would reopen the discriminated-envelope question ADR-108 has now rejected twice and must be re-decided there, not slipped in. No new prop, no new fetch, no chart-geometry change. *(Slice 05 replaces that sentence with one true whether the fill is on or off — "Nothing to show for the selected range. Days appear here as Lighthouse records them." — so the widgets never learn the mode.)*

**The fill ships opt-in (amendment 2026-09-24, slice 05; ADR-207 amendment).** Everything above happens only where a System Admin has switched it on: one instance-wide row in the existing optional-features list (key `OverTimeHistoryFill`, "Fill in past days on over-time charts"), seeded **off** on fresh and upgraded instances, flagged Preview, free, changed through the existing System-Admin-guarded write; no migration, no new route. The maintainer's reasons: the fill writes rows it never takes back, it works the past out against today's configuration, and it costs work when a chart opens. A narrow switch port reads the row by key **per use** (missing row = off, never cached, so switching needs no restart) and is asked in exactly two places — the reconciler, after it has found days and before it asks the filler (the only caller of `AskFor`, so UI and Lighthouse-Clients reads are covered alike), and the filler at the start of each pass, so an ask queued before the switch went off never starts a fill after it. A pass already running when it goes off finishes (≤ 90 days worked out, ≤ 10 s); days already filled stay — nothing is purged, and nothing could be singled out, since a filled day is indistinguishable from a recorded one. **Not governed by the switch**, and not to be gated later: the recorder's refusal of all-zero percentile rows, the as-of-that-day limits, the shared writers, the demo synthesiser's rows, the memo's refresh-event handlers, and the database maintenance gate — which keeps asking "is something writing?" rather than "is filling allowed?", because a pass finishing after "off" is still writing. With the switch off, both series endpoints behave exactly as before the story; ADR-108 needs no further amendment. Follow-ups outside the story: default on (#6083), remove the switch (#6084).

**RBAC / contract / clients / website / outcomes**: reads inherit the existing `MetricsController` gate ⇒ **no RBAC change**. Request and response shapes are byte-identical ⇒ **no CLI/MCP version gate**, and — the reachability question epic 5427 learned to ask at its slice 04 — `lighthouse-clients` `5bcb2a6` already exposes both endpoints and needs no work. No external integration ⇒ **contract testing (Pact) N/A**. **Website N/A** (free-tier, no marketing surface). **Outcome-collision check: PERFORMED, no collision.** `docs/product/outcomes/registry.yaml` holds OUT-1 (pace bands), OUT-2 (date-window translation) and OUT-3 (delivery team lanes); none is a snapshot-reconstruction specification. *This corrects the epic-5427 section above, whose "no outcomes registry in this repo ⇒ outcome-collision check N/A" is now **stale** — the registry exists.*

### ADR References (this feature)

- [ADR-207](./adr-207-read-triggered-reconstruction-on-its-own-filler.md): a series read asks for the days it is missing; a filler of its own does the work. Trigger in a query-port decorator; unit is `(owner, window)`; 90 days per pass plus a wall-clock budget; reconciliation memo for convergence; fill-if-absent + unique index for the race. `IUpdateQueueService` (single lane, reverted lanes, operator-task surface, DB-maintenance gate), `IUpdateExecutionLock` alone, a scheduler, inline fill and `Task.Run` all rejected with evidence. **ACCEPTED** 2026-09-22. **Amended** 2026-09-22 (the maintenance gate sees the filler, both directions) and 2026-09-24 (the opt-in switch: where it gates, the one-lookup read-path cost, in-flight behaviour; and the walk order corrected to oldest-first, cap on days worked out).
- [ADR-208](./adr-208-a-past-day-is-computed-by-the-recorders-own-code.md): one day-writer per table with two named write policies; family descriptors extracted; every today-anchor made an explicit as-of day defaulting to today; one absence gate for both paths. A second handler entry point, a duplicated-family service, a `WritePolicy` flag, leaving the anchor at today, and changing `BaselineValidationService` itself all rejected; refusing PBC reconstruction for pinned-baseline owners is kept as the named fallback. **ACCEPTED** 2026-09-22.
- [ADR-107](./adr-107-percentiles-recording-handler-on-refresh-events.md) **amended (story-6053)**: the "separate scheduled/background recorder" rejection is reconciled — each of its three complaints is about a *scheduler*, and reconstruction's trigger is the independent one that rejection said was missing. Also records where the per-day computation moves to, and that the percentile zero-write gate now applies to the recorder too.
- [ADR-108](./adr-108-percentiles-over-time-series-http-contract.md) **amended (story-6053)**: the slice-03b "**never a recompute trigger**" property no longer holds and is replaced by "read-only **in the response**". No new route, DTO or field; the envelope rejection and the client-side empty-state disambiguation both stand; the slice-03b known defect becomes rarer, not closed.
- [ADR-109](./adr-109-demo-percentiles-backfill-handler.md) **amended (story-6053)**: the rejection of real-tenant backfill **narrows to synthesis**. Recomputation from stored items is permitted; this handler stays demo-only and untouched, and fill-if-absent is how the two paths coexist on a demo instance (DoD 9, decided rather than discovered).
- Cross-refs [ADR-106](./adr-106-percentiles-over-time-snapshot-table-shape.md) (the unique natural key used as the concurrency backstop; the `NoHorizon = 0` sentinel and the append-only `MetricType` ordinal a reconstructed row must honour), [ADR-195](./adr-195-update-queue-is-three-lanes-one-channel-each.md) (**describes a reverted design** — read with the story-5877 note above before believing the lane count), [ADR-027](./adr-027-target-architecture-modular-monolith-domain-events-cqrs-lite.md) (the domain-event bus the recorders sit on).

### C4

System Context: **no delta** — the connector is never asked for trend data; reconstruction reads only work items Lighthouse already stored, which makes it connector-independent by construction (unmeasured for Jira, Linear and ServiceNow: the dev DB's Jira connection has no owner attached). Container delta: two gap-asking query decorators, one reconciler port, one hosted filler with its in-memory memo, and two snapshot day-writers extracted from the recording handlers. Level 3 is deliberately **not** produced — seven components in one bounded context, below the threshold, and every arrow is already named at Level 2. Full Container diagram + reuse analysis: `docs/feature/story-6053-reconstruct-over-time-history/feature-delta.md` → "Wave: DESIGN / [REF] C4".

### Open risks carried out of DESIGN
...
| I2 | **No field asserts something the check did not examine.** | No `lowerBound`/`upperBound` pair (a bounds pair claims an interval; on a non-contiguous sound set that claim is false). No boolean for the current setting's standing — it is `Inside \| Outside \| NotDetermined \| NotTested`: `NotDetermined` means the Team's own window was swept but none of its cells could be evaluated (it is in `unevaluatedWindowDays`); `NotTested` means the Team has no rolling window to test, with `currentSettingNotTestedReason: UsesFixedDates \| NotAPositiveLength \| null` saying why (`UsesFixedDates` wins when both apply). Invariant: `currentSettingWasTested == false` ⟺ standing `NotTested` ⟺ reason non-null. A window none of whose checks ran is listed in `unevaluatedWindowDays`, never in `soundWindowDays`, and makes the `determination` `SomeWindowsSound` rather than `AllWindowsAlike` (`NotEnoughEvidence` when no window was evaluated). A level's `reading` is `NotEvaluated \| NeverHeld \| AlwaysHeld \| SometimesHeld`, and an extreme is reported only when the level's own rate expected at least one check to go the other way; `SometimesHeld` is a plain fact, and the held and expected counts beside it carry the judgement. A window holds up when its 95% forecast held in more than half of the checks that ran on it; delivering more than the 50% forecast never counts against a window. *(Amended 2026-09-26, DES-14..DES-17; DES-14's rule and the name `SometimesHeld`, formerly `AboutRight`, confirmed by the maintainer the same day.)* |
| I3 | **The feature has no write path, and cannot acquire one by mistake.** | `ForecastRealityCheckService` takes the resolved `Team` as a *parameter* and is never injected with `IRepository<Team>`. It holds no reference to any `Save`/`Update`/`Add`. Enforced by ArchUnitNET. The check's controller is the one exception to the no-repository rule, because it has to look the Team up before it can hand it over. It is excused by its full name and nothing else, so any other reality-check type - in the API or behind it - is still held to the rule; a second rule holds the controller to reading, as it may call no repository method named `Save`, `Add`, `Update`, `Remove` or `Delete`; and the controller is still held to the no-database rule. *(Amended 2026-09-26.)* |
...
| I9 | **The Team's own sampling window is always one of the windows swept.** | `sampledWindowDays = sort(standardWindowDays ∪ {ThroughputHistory})`, so "is your setting sound" is a membership test over cells that were actually run rather than an inference about an untested value. Sixteen cells on the standard ladder, twenty off it. Added 2026-09-22 by the maintainer's reversal of the original recommendation. The exceptions are a Team with no rolling window to add - it forecasts from fixed dates, or its stored window is zero days or fewer - which gets the standard sixteen and the standing `NotTested` (I2). |
| I10 | **Every period is an exact run of days, and no check learns from a day it is scored on.** | Every date in a cell is a calendar day inside the stretch it bounds, first and last alike. For a horizon of H days and a window of W days ending on the instance's own day: scored `today − H + 1 .. today` (H days), learned from `today − H − W + 1 .. today − H` (W days, the count the live forecast reads for a W-day setting). The forecast covers the working days among the scored H. A Team finishing one Work Item every day therefore reads exactly `WithinBand` at every level. *(Added 2026-09-26, DES-19: the original formulas scored H + 1 days and learned from W + 1, one of them shared.)* |

### Component decomposition

**Backend** — four new types, one of them the check's own controller.

| Component | Verdict | Responsibility | Contract shape |
|---|---|---|---|
| `ForecastRealityCheckController.RunRealityCheck` | CREATE NEW | Driving adapter, on its own controller rather than on `ForecastController`. It carries the same `api/v1/forecast` + `api/latest/forecast` routes, so the URLs are those `ForecastController`'s routes would have given. Its three collaborators (`IForecastRealityCheckService`, `IRepository<Team>`, `ITeamMetricsService`) arrive through the constructor. Resolves the Team via the shipped `GetEntityByIdAnExecuteAction`, maps `applyFilterOverride` to a `ThroughputFilterMode` through `ThroughputFilterOverride.ToFilterMode`, the one mapping `ForecastController` uses too, returns the envelope. **No input validation** — the body carries no dates, so there is nothing to validate. **Why its own controller:** the maintainer prefers constructor injection, and `ForecastController` already takes seven constructor parameters — adding an eighth trips Sonar's S107, which here is a fair signal that the class carries too much. Injecting the service into the action with `[FromServices]`, or suppressing S107, would each have hidden that signal instead of answering it. | Adapter |
...
Items, **885 ms** at 5,000 and **1,745 ms** at 20,000 — and **exactly twenty reads of the Team's finished
work at every volume, not growing with Team size.** (Those are the reads of finished work, not every round
trip: the blackout days are read too, through a service the count stands in for.) Warm cost is flat at ~615 ms because the cache holds projected run charts rather
...
| Performance | **Measured, not open.** 701 ms cold median against a 5,000 ms budget on a real Team; twenty reads of the Team's finished work, flat in Team size; Monte Carlo floor dominates. An off-ladder Team adds four cells — extrapolated ~870 ms. See above. |
...
- **E5** — three assertions holding I1: `sampledWindowDays` equals `standardWindowDays` union the Team's
  ...
  while there is nothing to sort by. `unevaluatedWindowDays` is held to the same subsequence rule and never
  shares a member with `soundWindowDays` (amended 2026-09-26, DES-15).
- **E6** — sufficiency reason, cell outcome, determination, level reading (including `NotEvaluated`), the
  current setting's standing (including `NotTested`) and its not-tested reason render through exhaustive
...
| R-1 is unresolved | **Closed by measurement** — 701 ms cold / 612 ms warm on a real Team against a 5,000 ms budget; twenty reads of the Team's finished work, not growing with Team size. AC-1.1 stays in slice 01 but is now a confirmation on real hardware, not a gate that could change the design. |
...
**A read may ask; only the filler may write (ADR-207).** The trigger is the series read detecting a gap in the window it was asked for (D2), which overturns ADR-108's slice-03b "never a recompute trigger" property — amended there, narrowed to "read-only **in the response**". The seam is a **decorator over each series query port**, not the controller (four call sites and a controller that computes) and not the query implementation (a component whose name would stop describing it). The decorator delegates the read, hands the rows and the window to `IOverTimeGapReconciler` — one void method, no repository — and returns the rows untouched. **Nothing reachable from a controller action depends on a snapshot repository**, so "a GET wrote to the database on the request thread" is structurally unavailable rather than merely untested. Gap detection is a scan over rows already materialised plus a dictionary lookup; **no additional query runs on the read path**, and in particular the data floor is resolved inside the background pass, because the KPI budget is 50 ms over today's p95.

**The carrier is a filler of this feature's own, not `IUpdateQueueService` — the one place the reuse default was overridden, and it is evidence-backed.** `UpdateQueueService` holds a single `Channel<Func<Task>>` drained by a single reader: ADR-195's three-lane fix for exactly that starvation was shipped and **reverted** with story-5877 (see the note above), so ADR-195/196/197 describe a design that is not in the code. A 90-day multi-family pass — 5–6 s on 621 items, unbounded above that — would therefore sit in front of every Team and Portfolio refresh, ranking a cosmetic chart backfill ahead of the product's core function. Two further costs settle it: a new `UpdateType` is a **user-visible cancellable Task-Manager row** needing a verb, a noun and a terminology key in three `satisfies Record<UpdateTaskType, …>` tables plus a hand-maintained TypeScript union with no compile-time link to the backend enum; and an admitted key keeps `HasActiveWork()` true, which is what holds database maintenance off. What is reused is the *pattern* — bounded channel, single reader, per-key in-flight set, scope per pass, drain on shutdown.

**The unit of work is `(owner, window)`, never a family or a day.** One ask per owner per read; the pass covers every percentile family and every PBC family for the scope. That is what makes "switching tabs does not re-trigger reconstruction" a structural property rather than a behaviour to test for, and it is what makes every tab span the same period — the thing the combined widget was built for. **D4's cap is locked at 90 days *per pass*, not 90 days of reach**, plus a wall-clock budget: a year-wide picker fills across successive loads, and the walk is **resumable by construction** because the thing that triggers it is a read that will happen again. That is the honest answer to SPIKE-01's explicit inability to extrapolate cost from 621 items on SQLite to tens of thousands. Cost converges to zero once a window is dense; a per-owner in-memory **reconciliation memo** (resolved floor + refused days, invalidated by the refresh events) is what stops a permanently-unfillable day re-enqueueing a pass forever, and it is optimisation-only — losing it to a restart costs one wasted pass. Idempotency is three layers: the in-flight set at enqueue, **fill-if-absent** at write (never update-in-place — a recorded day is left exactly as recorded), and the ADR-106 unique index as the collision backstop, absorbed per day. Two replicas can both run a pass; that is wasted CPU, never a duplicate row.

**A past day is computed by the recorder's own code (ADR-208).** The per-day computation and the family descriptors — `CycleTimeHorizons`, `WorkItemAgeHorizons`, `TeamReaders`/`PortfolioReaders`, `LookbackDaysFor` — move out of the two recording handlers into two shared writers, each offering two **named** operations (`RecordToday` overwrites; `FillDayIfAbsent` writes only if absent) rather than one operation with a mode flag. One code path makes D1 and D6 structural instead of tested-for, and one family list per scope turns "the sets match `TeamReaders`/`PortfolioReaders` exactly" into an assertion about one list — the drift ADR-109's slice-02 amendment already recorded happening to the demo backfill's guard. **Every today-anchor on the PBC path becomes an explicit as-of day defaulting to `Clock.Today`**, threaded to `BaselineValidationService.Validate` and the `DoneItemsCutoffDays` cutoff: validating day D against today answers a different question, and its failure is silent — a pinned-baseline owner can go `BaselineInvalid`, hit the `Status != Ready` gate, and produce nothing that looks exactly like "the data didn't support it". A pinned baseline's correct series is **flat limits, not an empty one**. **The absence gate applies to both paths**, answering the adjacent question DISCUSS left to DESIGN: gating only reconstruction would make a day's row depend on which path reached it first, and fill-if-absent would make the recorder's all-zero row permanent — a standing invisible exception to D6. That is a **behaviour change to shipped code** (the forward recorder stops writing all-zero percentile rows) and is named in the release notes and the metrics docs; existing rows stay, with no repair migration.

**Frontend**: copy only, in slice 04. The forward-only empty-state sentence becomes false in two new ways, and both are covered by one sentence true of both states ("some days in this range have no recorded or reconstructible value") rather than by a `hasHistory` field — which would reopen the discriminated-envelope question ADR-108 has now rejected twice and must be re-decided there, not slipped in. No new prop, no new fetch, no chart-geometry change.

**RBAC / contract / clients / website / outcomes**: reads inherit the existing `MetricsController` gate ⇒ **no RBAC change**. Request and response shapes are byte-identical ⇒ **no CLI/MCP version gate**, and — the reachability question epic 5427 learned to ask at its slice 04 — `lighthouse-clients` `5bcb2a6` already exposes both endpoints and needs no work. No external integration ⇒ **contract testing (Pact) N/A**. **Website N/A** (free-tier, no marketing surface). **Outcome-collision check: PERFORMED, no collision.** `docs/product/outcomes/registry.yaml` holds OUT-1 (pace bands), OUT-2 (date-window translation) and OUT-3 (delivery team lanes); none is a snapshot-reconstruction specification. *This corrects the epic-5427 section above, whose "no outcomes registry in this repo ⇒ outcome-collision check N/A" is now **stale** — the registry exists.*

### ADR References (this feature)

- [ADR-207](./adr-207-read-triggered-reconstruction-on-its-own-filler.md): a series read asks for the days it is missing; a filler of its own does the work. Trigger in a query-port decorator; unit is `(owner, window)`; 90 days per pass plus a wall-clock budget; reconciliation memo for convergence; fill-if-absent + unique index for the race. `IUpdateQueueService` (single lane, reverted lanes, operator-task surface, DB-maintenance gate), `IUpdateExecutionLock` alone, a scheduler, inline fill and `Task.Run` all rejected with evidence. **ACCEPTED** 2026-09-22.
- [ADR-208](./adr-208-a-past-day-is-computed-by-the-recorders-own-code.md): one day-writer per table with two named write policies; family descriptors extracted; every today-anchor made an explicit as-of day defaulting to today; one absence gate for both paths. A second handler entry point, a duplicated-family service, a `WritePolicy` flag, leaving the anchor at today, and changing `BaselineValidationService` itself all rejected; refusing PBC reconstruction for pinned-baseline owners is kept as the named fallback. **ACCEPTED** 2026-09-22.
- [ADR-107](./adr-107-percentiles-recording-handler-on-refresh-events.md) **amended (story-6053)**: the "separate scheduled/background recorder" rejection is reconciled — each of its three complaints is about a *scheduler*, and reconstruction's trigger is the independent one that rejection said was missing. Also records where the per-day computation moves to, and that the percentile zero-write gate now applies to the recorder too.
- [ADR-108](./adr-108-percentiles-over-time-series-http-contract.md) **amended (story-6053)**: the slice-03b "**never a recompute trigger**" property no longer holds and is replaced by "read-only **in the response**". No new route, DTO or field; the envelope rejection and the client-side empty-state disambiguation both stand; the slice-03b known defect becomes rarer, not closed.
- [ADR-109](./adr-109-demo-percentiles-backfill-handler.md) **amended (story-6053)**: the rejection of real-tenant backfill **narrows to synthesis**. Recomputation from stored items is permitted; this handler stays demo-only and untouched, and fill-if-absent is how the two paths coexist on a demo instance (DoD 9, decided rather than discovered).
