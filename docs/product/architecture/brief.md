...
**RBAC / contract / website**: all unchanged. Free-tier (D3) ⇒ no RBAC gate change and **website N/A**; additive contract ⇒ **CLI/MCP client version gate N/A**; no external integration ⇒ **contract testing (Pact) N/A**; no outcomes registry in this repo ⇒ **outcome-collision check N/A** (recorded, not silently skipped).
...
- [ADR-109](./adr-109-demo-percentiles-backfill-handler.md): demo-gated backfill handler; real tenants forward-only; empty-state-only + real-tenant-backfill rejected. **ACCEPTED** 2026-07-23.
...
| Remaining PBC metric types on that toggle (WIA/WIP/CT/Arrivals/Feature-Size portfolio-only) | (US-05) | **DEFERRED slice-04** |
...
Enum-ordinal note (all slices): `MetricType` and `ProcessBehaviorMetricType` (shipped slice-03, currently `Throughput` only) are persisted as their **integer ordinal**. New members are **appended only**; reordering or renumbering silently re-maps every already-shipped snapshot row to a different metric family, with no compiler error, no failing test and no migration to review.
