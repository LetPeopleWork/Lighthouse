...
- [ADR-103](./adr-103-feature-blocked-semantics-per-portfolio.md): blocked is per-portfolio; spells keyed `(FeatureId, PortfolioId)`; Any-portfolio retained as a read projection; `FeatureDto` stops computing blocked (closes the ADR-067 follow-up). **PROPOSED — awaiting user confirmation** (resolves D8/D12).
- [ADR-104](./adr-104-feature-blocked-capture-seam-and-spell-lifecycle.md): pre-update verdict hoisted in `RefreshFeatures`; new `FeatureBlocked`/`FeatureUnblocked` events; departure sweep with empty-refresh guard. **PROPOSED — awaiting user confirmation**.
