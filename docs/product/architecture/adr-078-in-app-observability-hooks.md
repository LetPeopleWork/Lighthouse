...
**Status**: **Accepted (slice-05 DELIVERED 2026-06-20 — OTel confirmed: `/metrics` Prometheus + Serilog JSON stdout, both off by default; SPIKE outcome below)** (2026-06-16 — Morgan, Solution Architect; interaction mode PROPOSE. Inherits System Decision 6 / A5.)

> **Slice-05 SPIKE outcome (2026-06-20).** OTel chosen as decided here, `prometheus-net` fallback not needed. The overhead question is resolved structurally rather than by micro-benchmark: telemetry **off** registers **no** OpenTelemetry services (asserted by `Telemetry_DisabledByDefault_NoExporterNoBehaviourChange` — no `MeterProvider` in the container, `/metrics` unmapped), so the standalone single container carries zero added overhead (D1 / AC3). When **on**, OTel coexists cleanly with the existing Serilog pipeline (metrics via `AddOpenTelemetry().WithMetrics`, logs via a config-selected JSON console template) — one instrumentation surface, no second stack. OTLP traces were left out of this slice (no US-05 AC/scenario demands them); they remain a documented follow-up behind the same `Telemetry:*` gate. Implemented as `Startup/TelemetryConfigurator` + `Startup/LoggingConfigurator`, config record `Configuration/TelemetryConfiguration` (`Telemetry:Enabled`, `Telemetry:Logging:Format`).
**Date**: 2026-06-16
