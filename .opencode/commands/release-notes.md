...
All ADO access is via the **`az` CLI** (`azure-devops` extension) through `bash` — **not** the ADO MCP server, which is not in use here (do not assume ADO MCP tools exist). See `.claude/commands/ado-sync.md` or `.opencode/commands/ado-sync.md` for the full cookbook.
...
Use `question` with up to 3 questions in a single call:
