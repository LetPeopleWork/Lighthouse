...
if ((Get-Process pcscd -ErrorAction SilentlyContinue) -eq $null) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Error "Failed to start pcscd automatically: $startResult"
        Write-Host "Try running 'sudo systemctl start pcscd' manually, then re-run this script."
        exit 1
    }
    ...
    Write-Host "pcscd started successfully."
}
