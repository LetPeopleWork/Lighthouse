import type { IMilestone } from "../Project/Milestone";
import {
	BurndownEntry,
	LighthouseChartData,
	LighthouseChartFeatureData,
	featureColors,
} from "./LighthouseChartData";
...
describe("LighthouseChartFeatureData Class", () => {
	...
	beforeEach(() => {
		...
		feature = new LighthouseChartFeatureData(
			"Feature 1",
			forecasts,
			burndownEntries,
		);
	...
	it("should not assign a color already used", () => {
		...
		const anotherFeature = new LighthouseChartFeatureData(
			"Feature 2",
			forecasts,
			burndownEntries,
		);
