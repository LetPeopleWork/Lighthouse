...
import { markerColors, markerLabels, STATUS_COLORS } from "./timelineMarkers";
...
describe("the colours a bar can wear", () => {
	it("gives each of the three a colour of its own", () => {
		...
		const { finished, startsAfterTarget, endsAfterTarget } = STATUS_COLORS;
	...
	it("takes them from the palette this product already reads forecasts in", () => {
		...
		expect(STATUS_COLORS.finished).toBe("#388e3c");
		expect(STATUS_COLORS.startsAfterTarget).toBe("#f44336");
		expect(STATUS_COLORS.endsAfterTarget).toBe("#ff9800");
	});
	...
	it("shares the target column's colour, on purpose", () => {
		...
		// to be told apart by shape and position, and by the key that names the colour.
		//
		// If looking at a real chart says they cannot be told apart, the answer is to move the
		// *column* to a colour nothing else uses and leave these alone - not to invent a second
		// amber here, which would leave the chart with two colours that both mean late.
		expect(STATUS_COLORS.endsAfterTarget).toBe(
			markerColors(
				createTheme({
