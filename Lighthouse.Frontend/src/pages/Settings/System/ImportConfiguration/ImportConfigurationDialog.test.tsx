  ...
  default: (props) => (
    <div data-testid="mock-import-settings-step">
      <button 
        ...
        onClick={() => props.onNext([{ id: 1 }], [], [], [], [], [], new Map(), new Map(), false)}
      >
        Next with WTS
      </button>
      <button
        ...
        onClick={() => props.onNext([], [], [], [], [], [], new Map(), new Map(), false)}
      >
        Next
      </button>
      <button type="button" data-testid="import-settings-close" onClick={props.onClose}>
        Close
      </button>
    </div>
  )
  ...
  default: (props) => (
    <div data-testid="mock-import-step">
      <button
        ...
        onClick={() => props.onNext({ workTrackingSystems: [], teams: [], projects: [] })}
      >
        Import
      </button>
      <button type="button" data-testid="import-cancel" onClick={props.onCancel}>
        Cancel
      </button>
    </div>
  )
...
vi.mock("./Steps/ImportSummaryStep", () => ({
  default: (props) => (
    <div data-testid="mock-import-summary-step">
      ...
      <button type="button" data-testid="import-summary-close" onClick={props.onClose}>
        Close
      </button>
    </div>
  )
...
describe("ImportConfigurationDialog Component", () => {
  ...
  it("should not render the dialog when open is false", () => {
    ...
    expect(screen.queryByTestId("import-configuration-dialog")).not.toBeInTheDocument();
  });
  ...
  it("should render the stepper with all steps", () => {
    ...
    const steps = ["File Selection", "Secrets Configuration", "Import", "Summary"];
  ...
  it("should advance to WorkTrackingSystemConfigurationStep when ImportSettingsStep completes with newWorkTrackingSystems", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("mock-work-tracking-system-config-step")).toBeInTheDocument();
    });
  });
  ...
  it("should advance to ImportStep when WorkTrackingSystemConfigurationStep completes", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("work-tracking-system-next")).toBeInTheDocument();
    });
  ...
  it("should cancel and reset state when cancel is clicked on WorkTrackingSystemConfigurationStep", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("work-tracking-system-cancel")).toBeInTheDocument();
    });
  ...
  it("should advance to ImportSummaryStep when ImportStep completes", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("mock-import-summary-step")).toBeInTheDocument();
    });
  });
