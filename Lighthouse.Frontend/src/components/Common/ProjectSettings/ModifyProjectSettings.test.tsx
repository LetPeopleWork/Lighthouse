...
vi.mock('../../../pages/Projects/Edit/AdvancedInputs', () => ({
    __esModule: true,
    default: ({ onProjectSettingsChange }: { onProjectSettingsChange: (key: keyof IProjectSettings, value: string) => void }) => (
        <div>
            <div>AdvancedInputsComponent</div>
            <button onClick={() => onProjectSettingsChange('name', 'value')}>Change Advanced</button>
        </div>
    ),
}));
...
vi.mock('../../../pages/Projects/Edit/GeneralInputs', () => ({
    ...
    default: ({ onProjectSettingsChange }: { onProjectSettingsChange: (key: keyof IProjectSettings, value: string) => void }) => (
        <div>
            ...
            <button onClick={() => onProjectSettingsChange('name', 'value')}>Change General</button>
        </div>
    ),
}));
...
vi.mock('../WorkItemTypes/WorkItemTypesComponent', () => ({
    __esModule: true,
    default: ({ onAddWorkItemType, onRemoveWorkItemType }: { onAddWorkItemType: (type: string) => void, onRemoveWorkItemType: (type: string) => void }) => (
        <div>
            <div>WorkItemTypesComponent</div>
            <button onClick={() => onAddWorkItemType('New Work Item')}>Add Work Item</button>
            <button onClick={() => onRemoveWorkItemType('Existing Work Item')}>Remove Work Item</button>
        </div>
    ),
}));
...
vi.mock('../WorkTrackingSystems/WorkTrackingSystemComponent', () => ({
    __esModule: true,
    default: ({ onWorkTrackingSystemChange, onNewWorkTrackingSystemConnectionAdded }: { onWorkTrackingSystemChange: (event: React.ChangeEvent<{ value: unknown }>) => void, onNewWorkTrackingSystemConnectionAdded: (connection: IWorkTrackingSystemConnection) => void }) => (
        <div>
            <div>WorkTrackingSystemComponent</div>
            <button onClick={() => onWorkTrackingSystemChange({ target: { value: 'System 1' } } as React.ChangeEvent<{ value: unknown }>) }>Change Work Tracking System</button>
            <button onClick={() => onNewWorkTrackingSystemConnectionAdded({ id: 1, name: 'New System', options: [], workTrackingSystem: '' })}>Add New Work Tracking System</button>
        </div>
    ),
}));
...
vi.mock('../StatesList/StatesList', () => ({
    __esModule: true,
    default: ({ onAddToDoState, onRemoveToDoState, onAddDoingState, onRemoveDoingState, onAddDoneState, onRemoveDoneState }: { onAddToDoState: (state: string) => void, onRemoveToDoState: (state: string) => void, onAddDoingState: (state: string) => void, onRemoveDoingState: (state: string) => void, onAddDoneState: (state: string) => void, onRemoveDoneState: (state: string) => void }) => (
        <div>
            <div>StatesList</div>
            <button onClick={() => onAddToDoState('New ToDo')}>Add ToDo</button>
            <button onClick={() => onRemoveToDoState('Existing ToDo')}>Remove ToDo</button>
            <button onClick={() => onAddDoingState('New Doing')}>Add Doing</button>
            <button onClick={() => onRemoveDoingState('Existing Doing')}>Remove Doing</button>
            <button onClick={() => onAddDoneState('New Done')}>Add Done</button>
            <button onClick={() => onRemoveDoneState('Existing Done')}>Remove Done</button>
        </div>
    ),
}));
...
describe('ModifyProjectSettings', () => {
    ...
    [
        ...
        { invalidValue: 'default Amount of Work Items', defaultAmountOfWorkItemsPerFeature: undefined },
