...
const FIELD_DISPLAY_NAMES: Record<string, string> = {
	...
	"workitem.name": "Name",
	"workitem.referenceid": "Reference ID",
	"workitem.parentreferenceid": "Parent Reference ID",
	"workitem.tags": "Tags",
};
...
const formatFieldName = (fieldKey: string): string => {
	...
	if (normalized.startsWith(ADDITIONAL_FIELD_PREFIX)) {
		const idPart = normalized.slice(ADDITIONAL_FIELD_PREFIX.length);
		return `Custom field ${idPart}`;
	}
	return fieldKey;
};
...
const formatOperator = (
	operator: string,
): { phrase: string; quote: boolean } => {
	switch (operator.toLowerCase()) {
		case "equals":
			return { phrase: "=", quote: false };
		case "notequals":
			return { phrase: "!=", quote: false };
		case "contains":
			return { phrase: "contains", quote: true };
		default:
			return { phrase: operator, quote: false };
	}
};
...
export const formatConditions = (
	...
	conditions
		.map((c) => {
			const field = formatFieldName(c.fieldKey);
			const op = formatOperator(c.operator);
			const value = op.quote ? `"${c.value}"` : c.value;
			return `${field} ${op.phrase} ${value}`;
		})
		.join("; ");
