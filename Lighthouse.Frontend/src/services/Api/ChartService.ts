...
export class ChartService extends BaseApiService implements IChartService {
	async getLighthouseChartData(
		...
		return this.withErrorHandling(async () => {
			const response = await this.apiService.post<ILighthouseChartData>(
	...
	private deserializeLighthouseChartData(
		lighthouseChartData: ILighthouseChartData,
	): LighthouseChartData {
		const featureData = lighthouseChartData.features.map(
			this.deserializeLighthouseChartFeatureData,
		);

		const milestones = lighthouseChartData.milestones.map(
	...
	private deserializeLighthouseChartFeatureData(
		featureData: ILighthouseChartFeatureData,
	) {
		...
		const forecasts = featureData.forecasts.map(
			(forecast) => new Date(forecast),
		);

		return new LighthouseChartFeatureData(
			featureData.name,
			forecasts,
			remainingItems,
		);
