...
import { LighthouseChartData, LighthouseChartFeatureData, BurndownEntry } from '../../models/Charts/LighthouseChartData';
...
describe('ChartService', () => {
    ...
    it('should get lighthouse chart data for a project', async () => {
        ...
        const mockResponse = {
            features: [
                {
                    name: 'Feature 1',
                    remainingItemsTrend: [{ date: '2024-01-02T00:00:00Z', remainingItems: 10 }],
        ...
        const result = await chartService.getLighthouseChartData(projectId, startDate, sampleRate);
        ...
        expect(mockedAxios.post).toHaveBeenCalledWith(`/lighthousechart/${projectId}`, {
            startDate: startDate,
    ...
    it('should throw an error if API call fails', async () => {
        ...
        await expect(chartService.getLighthouseChartData(projectId, startDate, sampleRate))
            .rejects
            .toThrow('API error');
