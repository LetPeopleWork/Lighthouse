...
export class BaseApiService {
    ...
    protected static deserializeTeam(item: ITeam) {
        ...
        return new Team(item.name, item.id, projects, features, item.featureWip, item.actualFeatureWip, new Date(item.lastUpdated));
    }

    protected static deserializeProject(item: IProject): Project {
        const features = BaseApiService.deserializeFeatures(item.features);
        const teams = item.involvedTeams.map(team => new Team(team.name, team.id, [], [], team.featureWip, team.actualFeatureWip, new Date(item.lastUpdated)));
