...
> - `adr-001 … adr-208` — point decisions (in that folder). The index at the end maps the load-bearing ones, and says which are designed but not built.
>
...
**Background refresh (the write/sync path).** A scheduled `*Updater` (e.g. `PortfolioUpdater`, `TeamUpdater`) runs on the **single-reader update queue**: pull work items from the connector, reconcile the local projection, recompute deliveries/forecasts, write back. Because one reader drains the channel, operations on the same entity are serialised — this is the canonical "exclusive operation on entity X" primitive (use `IUpdateQueueService.EnqueueUpdate` / `EnqueueAndAwaitAsync`, never a parallel semaphore). Admission goes through `IUpdateStatusStore.TryAdmit` and execution through `IUpdateExecutionLock`, so in a multi-replica deployment "at most one active lifecycle per `UpdateKey`" is a *fleet-wide* invariant held by a Postgres advisory lock rather than a process-local one (ADR-076). A rejected admission parks a single coalesced follow-up and `Requeue`s the key, so the key never reads as idle during the handover. The sync reports its own scope back to the updater as a `SyncOutcome` (mode, records scanned, records fetched), sourced where the fetch happens rather than counted afterwards; the updater persists it on the `RefreshLog` row and emits one Information-level summary line per completed update. Everything the update *iterated over* is Debug (Epic #5687). How much a refresh downloads is decided per cycle by the pure `SyncModeResolver` (ADR-138): with a connector that answers `SupportsIncrementalSync(connection)` (ADR-139 — per connection, because Jira Cloud and Data Center are one class deciding deployment at runtime), the fetch runs in two phases — sweep the whole query for `(referenceId, changedAt)`, then download payloads only for the records whose stamp moved. Removal stays a set difference against the **swept** set, never against what was downloaded, so nothing about deletion changes with the mode. Anything ambiguous — connector cannot sweep, sweep failed, a stored record without a stamp — resolves to a full download; there is no partial mode. Time-driven derivations (staleness, rollup, extrapolation, forecasts) run over the *stored* set every cycle regardless of mode (ADR-141), because the record that stops being fetched is exactly the record that goes stale.

**Where a parent comes from is the connector's to declare.** Most trackers answer with their own parent field, or with a field whose value is a key. A Jira instance that records hierarchy as issue links — "caused by", "results in" — has no field to read at all. Rather than teaching the reconciler about link types, the connector declares which source applies for a given query owner and one pure `ParentSourceSelector` resolves it, so the shape of the answer is the same whichever source produced it (ADR-193, applying ADR-071's connector-capability pattern to a second capability; distinct from ADR-157, which answers the adjacent question for a Portfolio's dependency references and deliberately does not share its type). Direction is inferred rather than configured, because Jira serves every link from both ends. A record whose links match two or more candidates takes **no** parent and names them: a parent is 0..1, and a silently wrong one corrupts a Feature's size while reading as good data.
...
Not everything time-dependent earns a snapshot table. The chance an open Work Item misses its Team's Service Level Expectation is a **pure conditional over that Team's closed cycle times**: of every item that was still open at this item's age, the share that went on to exceed the target (ADR-192). It takes the age from its caller rather than reading a clock, which keeps it a function of its three arguments; past the target the answer is 100% whatever the history holds.

It is **a number per item, produced by the read that asks for it** — not a background ladder on a schedule, and not a column on the work-item table (ADR-194). There is no way to express "no answer": an item nothing can be said about is left out of the collection by the caller instead of carrying a null every reader has to interpret. One derivation feeds the aging chart's zones, the dialog's column and the at-risk widget, so those three cannot disagree, and the figure is published alongside how much finished work it rests on — a fact about the history rather than about how the risk was derived, which is what lets one such number stay true beside every answer the calculation can give.
...
- Reusable hooks own cross-cutting client state: **`useRbac`** (authorization gating), **`useModifySettings`** (the config auto-save state machine — debounced save, `idle/saving/saved/error/conflict` states, optimistic-concurrency-token chaining, in-flight serialisation; ADR-029/030), and **`useDebouncedRevisionRun`** (settle a burst of user steps and run the effect once, keyed by a revision counter rather than by the values it reads). The metrics time window is owned by a hook of its own rather than by the view that renders it, so the presets, the steppers and the date pickers all move it through one write path (ADR-189).
...
- **A third-party UI component is confined to one adapter, and a test keeps it there.** The Delivery timeline is the first place this repo draws on a rendering library it does not own (`@svar-ui/react-gantt`, MIT). Exactly one component imports it; the component's props speak in this product's nouns — bars, days, a target date — and no type of the library's appears on its surface. Above that line nothing knows which library draws the chart, so replacing it is a rewrite of one file. The property decays silently, because a second import compiles and reads fine in review, so an enforcement test scans the source tree and fails naming any other importer. Two consequences worth knowing before adding the next such component: its markup is **not** asserted on anywhere (a test reaching into it goes red on their release rather than on our defect, so the component is stood in for and what is pinned is the contract handed to it), and the one test that does render it for real needs a canvas stub, because the library paints to a canvas and jsdom has none.
- **A shared dialog's optional columns travel with the population that owns them** (ADR-198). The work-item dialog is rendered from several payloads, and a column that describes the items — a pace band, a risk figure — is only meaningful for the population it was computed over. So where one function builds many payloads, the in-flight population and its descriptors are assembled once in a named base the payloads spread, three characters apart from each other; where a component owns exactly one population and has exactly one caller, the descriptor's inputs are a **required** prop, so omitting them is a compile error rather than a column that quietly disappears.
...
| 132 – 136 | Feature ordering: a derived total order (no ordering aggregate), rank-change domain event, ordering-policy setting (187 moved where that setting is stored) |
...
| **187 – 189** | **Behaviour settings hold the Feature-ordering choice, with a per-key applier carrying its consequence (§10, superseding 134 on storage); one pace-band ladder read by both the aging chart's zones and the dialog's band column; the metrics time window owned by a hook, debounced on a revision counter (§12)** |
| **190 / 191** | **Usage data: events detected in the browser and forwarded by the backend so the collector's address and key stay server-side, and an analytics identity that is a per-browser pseudonym the browser never sees and that never travels on the wire (§10)** |
| **192 / 194 / 198** | **SLE risk: a pure conditional over the Team's closed cycle-time population, a number per item produced by the read rather than a background ladder (§6), and a shared dialog's optional columns attached by the payload that owns the population (§12)** |
| **193** | **Parent source is a declared connector capability — a link type read where a field name goes, resolved by one pure selector (§3). Applies 071's pattern; distinct from 157** |
| 195 – 197 | Update-queue lanes, the write-back round's concurrency contract and an in-process wall-time deadline. **Accepted in DESIGN, built, then reverted before release** — overlapping refreshes lost Feature-to-Team ownership. Nothing in the code implements them; the queue is still the single-reader channel of §3 |
| **199 – 202** | **Forecasted start dates: the start day observed inside each trial at two grains, stored in its own collection, an observed start superseding the forecast as a rule on `Feature`, and a run that forecasts the board as configured (§3)** |
...
| **207 / 208** (reconstruction) | **Over-time history fill: a series read that finds missing days asks a filler of its own, off the update queue, behind an opt-in behaviour setting; a past day is computed by the recorder's own code, written only if absent (§6)** |
| 207 / 208 (reality check) | The Forecast Reality Check: a report is a response, not a stored record, and a forecast level either holds or does not. **Accepted in DESIGN, not built.** The two numbers were issued twice by parallel DESIGN waves; the file name tells the pairs apart |

The full set (001–208 — 173–177, 181–186, 195–197, 206 and the reality-check 207 / 208 are *Proposed*, reverted or not yet built: designed, but not what the code does), the per-feature DESIGN deltas ([`brief.md`](docs/product/architecture/brief.md)), and the diagrams ([`c4-diagrams.md`](docs/product/architecture/c4-diagrams.md)) all live under [`docs/product/architecture/`](docs/product/architecture/).
