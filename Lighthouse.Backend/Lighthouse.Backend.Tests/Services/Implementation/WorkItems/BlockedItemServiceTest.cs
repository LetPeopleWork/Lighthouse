...
namespace Lighthouse.Backend.Tests.Services.Implementation.WorkItems
{
    ...
    public class BlockedItemServiceTest
    {
        ...
        private static Team TeamWithAdditionalFields(string blockedRuleSetJson, params int[] definedFieldIds)
        {
            ...
            foreach (var id in definedFieldIds)
            {
                connection.AdditionalFieldDefinitions.Add(new AdditionalFieldDefinition { Id = id, DisplayName = $"Field {id}" });
            }

            return new Team
            {
                BlockedRuleSetJson = blockedRuleSetJson,
