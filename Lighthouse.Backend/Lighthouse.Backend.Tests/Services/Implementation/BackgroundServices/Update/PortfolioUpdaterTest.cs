        ...
        public void TriggerUpdate_Cancelled_RecordsTheRefreshAsCancelledRatherThanFailed()
        {
            ...
            using (Assert.EnterMultipleScope())
            {
                Assert.That(recordedRefresh?.Cancelled, Is.True,
                    "The portfolio half logs its own refresh, so it can report a cancel as a failure "
                    + "independently of the team half.");

                Assert.That(recordedRefresh?.Success, Is.False,
                    "Still not a refresh that finished - a cancelled run counted as successful would "
                    + "inflate the success rate with runs nobody ran.");
