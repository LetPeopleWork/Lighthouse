...
{
    [NonParallelizable]
    public class S1_CorsFailClosedTests
    {
        private const string AnonymousProbeEndpoint = "/api/latest/version/current";
        private const string ConfiguredOrigin = "https://example.org";
        ...
        [Test]
        public async Task S1_AuthDisabled_AppStartsWithWildcardCors_RegressionGuard()
        {
            Environment.SetEnvironmentVariable("Authentication__Enabled", "false");

            using var factory = new TestWebApplicationFactory<Program>();
            using var client = factory.CreateClient();

            using var request = BuildPreflightRequest(AnonymousProbeEndpoint, "https://anyhost.example.com");
            using var response = await client.SendAsync(request);

            Assert.That(response.IsSuccessStatusCode, Is.True, $"Expected preflight success, got {(int)response.StatusCode}");
            var allowOrigin = ReadHeader(response, "Access-Control-Allow-Origin");
            Assert.That(allowOrigin, Is.EqualTo("*"));
        }

        [Test]
        public void S1_AuthEnabledAndAllowedOriginsEmpty_HostFailsToStartWithFatalLog()
        {
            SetAuthEnabledEnv();
        ...
        [Test]
        public async Task S1_AuthEnabledWithConfiguredOrigin_PreflightForOriginReturns204()
        {
            SetAuthEnabledEnv();
            Environment.SetEnvironmentVariable("Authentication__AllowedOrigins__0", ConfiguredOrigin);
        ...
        [Test]
        public async Task S1_AuthEnabledWithConfiguredOrigin_PreflightForForeignOriginFails()
        {
            SetAuthEnabledEnv();
            Environment.SetEnvironmentVariable("Authentication__AllowedOrigins__0", ConfiguredOrigin);
