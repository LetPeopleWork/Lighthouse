...
namespace Lighthouse.Backend.Services.Implementation.WorkTrackingConnectors.Jira
{
        ...
        : IJiraWorkTrackingConnector
    {
        ...
        private const string OutwardLabelProperty = "outward";
            ...
            IWorkItemQueryOwner owner, IEnumerable<string> sweepQueries, string sweptDescription, CancellationToken cancellationToken)
        {
            ...
            foreach (var sweepQuery in sweepQueries)
            {
                ...
                {
                    LogTheRefusal(sweepQuery, rejection);
        ...
        public async Task<ConnectionValidationResult> ValidateConnection(WorkTrackingSystemConnection connection)
        {
            ...
            try
            {
                ...
                {
                    return TheVerdictOnReferencesThatResolvedToNothing(referencesThatResolvedToNothing, resolution.LinkTypes);
                }
        ...
        {
            var references = string.Join(", ", referencesThatResolvedToNothing);

            if (linkTypes.RefusedWith is { } refusal)
            {
                return ConnectionValidationResult.Failure(
            ...
            return ConnectionValidationResult.Failure(
                "additional_fields_invalid",
                $"Some additional fields could not be found: {references}. This Jira defines no field and no issue link type of that name. The issue link types it does define are: {string.Join(", ", names)}.",
        ...
        /// <summary>
        /// A query Jira refused is not an unexpected error - we know exactly what went wrong, because Jira
        /// said so. The headline is what a user reads first, so it has to name the query rather than blame
        /// something unnamed, and Jira's own sentence goes underneath it where the detail belongs. The throw
        /// site has already written the query and the whole response body to the log.
        /// </summary>
        private static ConnectionValidationResult QueryWasRejected(JiraQueryRejectedException rejection)
            ...
            Dictionary<string, ResolvedReference> customFieldReferences, LinkTypesMayAnswer mayAnswer)
        {
            ...
            // type look like two candidates and go unresolved on a correct configuration. Two genuinely
            // different types answering is a tie that stays undecided: choosing either would quietly read
            // the wrong links.
            foreach (var reference in referencesThatResolvedToNothing)
            {
                var answeringTypes = listing.Types.FindAll(linkType => linkType.AnswersTo(reference));
        ...
        /// <summary>
        /// What each Additional Field reference resolved to, alongside what the instance offered while
        /// resolving them. The listing is empty both when the instance named no link types and when nothing
        /// was left for the link types to answer, which is why only a resolution carrying a reference that
            ...
            Dictionary<string, List<string>> carriedWork)
        {
            try
            {
                ...
                var rejection = transport == JiraDeployment.Cloud
