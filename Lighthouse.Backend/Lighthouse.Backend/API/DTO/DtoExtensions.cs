...
namespace Lighthouse.Backend.API.DTO
{
    ...
    {
        public static IEnumerable<WhenForecastDto> CreateForecastDtos(this ForecastBase forecast, DateOnly today, IReadOnlyList<BlackoutPeriod> blackoutPeriods, params int[] probabilities)
        {
            var forecastDtos = new List<WhenForecastDto>();
            foreach (var probability in probabilities)
            {
                forecastDtos.Add(
                        new WhenForecastDto(forecast, probability, today, blackoutPeriods)
                    );
            }
