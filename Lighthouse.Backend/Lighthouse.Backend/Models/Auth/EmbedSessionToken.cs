...
namespace Lighthouse.Backend.Models.Auth
{
    ...
    public class EmbedSessionToken : IEntity
    {
        ...
        // ADR-132 D65: a row is either a grant (TokenId + SecretHash) or a refusal (RefusalCode),
        // never both — enforced by CK_EmbedSessionTokens_GrantOrRefusal, not by convention.
        public string? TokenId { get; set; }

        public string? SecretHash { get; set; }

        // ADR-132 D63: the API-key path stays mintable this release, so the column stays and turns optional.
        ...
        // ADR-132 D68 amends D55: consumption stamps this instead of clearing the hash, so a second
